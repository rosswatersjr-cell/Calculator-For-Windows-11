...
class MyFontChooser(ctk.CTkToplevel):
    def __init__(self, parent, font_dict={}, text="Sample text for selected font", **kwargs):
        ...
        self.withdraw()
        if os.path.exists("Font_Families.json"):# Font Family Names are stored in json file. Not reliant on tk.
...
class MyDialog(ctk.CTkToplevel):
    ...
    def on_cancel(self):
        ...
        self.destroy() # Close the dialog
        return self.result    
class TimedPopup(ctk.CTkLabel): # The Popup Is Centered On The Parent Widget. 
    def __init__(self, parent, text, text_color, fg_color, font_size, delay_time): 
        ...
        try:
            fake_lbl= ctk.CTkLabel(master=parent, text=text, font=self.popup_font)# Just To Get Required Width And Height
            parent.update()
            self.req_width=fake_lbl.winfo_reqwidth()
            self.req_height = fake_lbl.winfo_reqheight()
            self.scale_factor = ctypes.windll.shcore.GetScaleFactorForDevice(0) / 100
            parent_wid=parent.winfo_width() / self.scale_factor
            parent_center_x=(parent_wid / 2)
            self.x_pos=abs((parent_center_x - (self.req_width / 2)))
            ...
            self.y_pos=abs((parent_center_y-(self.req_height / 2))) + add_y
            fake_lbl.destroy()    
        ...
        try:
            for i in range(len(self.caption_list)):# Get Max Text Length For Menu Width
                ...
                if len(txt)>self.max_wid:
                    ...
                    self.text = txt
            fake_btn= ctk.CTkLabel(self, text=self.text, font=self.menu_font)# Just To Get Required Width And Height
            parent.update()
            self.update()
            self.req_width=fake_btn.winfo_reqwidth()
            self.req_height=fake_btn.winfo_reqheight() * len(self.caption_list)
            fake_btn.destroy()    
            ...
            self.menu_items=[] 
            for i in range(0, len(self.caption_list)):# Get Max Text Length For Menu Width
                ...
                self.menu_items[i].bind("<Button 1>", lambda e: self.exit())
...
class Calculator(ctk.CTk):
    ...
    def button_popups(self, event, txt):# display the popup menu
        ...
        btn_popup = TimedPopup(self.Display_Text, text=text, text_color="#000000", 
                            fg_color="#14fafa", font_size=12, delay_time=1000)
    ...
    def show_menu_popup(self, event):# app
        ...
        self.popup_menu = CustomPopupMenu(self, captions ,commands, 16, self.winfo_pointerx(), self.winfo_pointery())
    ...
    def give_greeting(self, status):
        ...
        self.greet_popup = TimedPopup(self. Display_Text, text=greeting, text_color="#000000", 
                            fg_color="#14fafa", font_size=12, delay_time=1500)
