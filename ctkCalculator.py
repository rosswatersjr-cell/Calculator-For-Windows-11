...
version="2026.09.26"
class rwColorPicker(ctk.CTkToplevel):
    def __init__(self, master=None, init_color="#00aaff", title="Choose Color"):
        ...
        self.base_font_size = int(-20 * self.factor) 
        self.hex_font_size = int(-26 * self.factor) 
        self.bind("<Configure>", self.on_resize)
        self._resize_after_id = None
        self.chooser_font = ctk.CTkFont(family="Arial", size=self.base_font_size, weight="normal", slant="roman")
        self.hex_font = ctk.CTkFont(family="Arial", size=self.hex_font_size, weight="normal", slant="roman")
        self.hue = 0.0
        self.sat = 1
        self.val = 1
        self.result = None
        ...
        self.marker_radius = 10 * self.scale
        self.canvas_height = self.screen_height * 0.2
        self.canvas_width = self.canvas_height * ratio
        self.hex_color = init_color
        self.canvas_picker = ctk.CTkCanvas(self, width=self.canvas_width, height=self.canvas_height, highlightthickness=0)
        self.canvas_picker.pack(expand=True, fill="both", anchor='nw', pady=10, padx=10)
        self.canvas_picker.bind("<Button-1>", self.picker_clicked)
        self.canvas_picker.bind("<B1-Motion>", self.picker_clicked)
        self.canvas_picker.bind("<Configure>", lambda e: self.update_canvas_picker())
        if Language == 'Spanish':
            slider_lbls = ['Matiz', 'Rojo', 'Verde', 'Azul']
            self.btn_lbls = ['Vale', 'Cancelar']
        else:                
            slider_lbls = ['Hue', 'Red', 'Green', 'Blue']
            self.btn_lbls = ['OK', 'Cancel']
        rgb_frame = ctk.CTkFrame(self)
        rgb_frame.pack(expand=True, fill='both', pady=10, padx=10)
        self.hue_slider = ctk.CTkSlider(rgb_frame, from_=0, to=1, height=25,  fg_color='yellow', width=self.canvas_width,
                                        button_hover_color='aqua',  progress_color='purple', command=self.hue_change)
        self.r_slider = ctk.CTkSlider(rgb_frame, from_=0, to=255, height=25, fg_color='gray', width=self.canvas_width, 
                                      button_hover_color='aqua', progress_color='red', command=self.red_change)
        self.g_slider = ctk.CTkSlider(rgb_frame, from_=0, to=255, height=25, fg_color='gray', width=self.canvas_width, 
                                      button_hover_color='aqua', progress_color='green', command=self.green_change)
        self.b_slider = ctk.CTkSlider(rgb_frame, from_=0, to=255, height=25, fg_color='gray', width=self.canvas_width, 
                                      button_hover_color='aqua', progress_color='blue', command=self.blue_change)
        self.hue_lbl = ctk.CTkLabel(rgb_frame, text='Hue',  font=self.chooser_font, width=self.canvas_width)
        self.hue_lbl.pack()
        self.hue_slider.pack(expand=True, fill="x", padx=20)
        self.red_lbl = ctk.CTkLabel(rgb_frame, text=slider_lbls[1], font=self.chooser_font)
        self.red_lbl.pack(expand=True, fill="x",padx=20)
        self.r_slider.pack(expand=True, fill="x", padx=20)
        self.green_lbl = ctk.CTkLabel(rgb_frame, text=slider_lbls[2], font=self.chooser_font)
        self.green_lbl.pack(expand=True, fill="x", padx=20)
        self.g_slider.pack(expand=True, fill="x", padx=20)
        self.blue_lbl = ctk.CTkLabel(rgb_frame, text=slider_lbls[3], font=self.chooser_font)
        self.blue_lbl.pack(expand=True, fill="x", padx=20)
        self.b_slider.pack(expand=True, fill="x", padx=20)
        self.preview = ctk.CTkFrame(self, width=self.canvas_width, height=50, border_width=3, border_color='cyan', corner_radius=10)
        self.preview.pack(expand=True, pady=(20, 10))
        self.hex_label = ctk.CTkLabel(self, text="#000000", font=self.hex_font, width=self.canvas_width)
        self.hex_label.pack(expand=True, fill="x",pady=5)
        btn_frame = ctk.CTkFrame(self)
        btn_frame.pack(expand=True, fill='both', pady=10, padx=10)
        # Grid Buttons Inside Packed Frame For Equal Resizing
        btn_frame.grid_columnconfigure(0, weight=1)
        btn_frame.grid_columnconfigure(1, weight=1)
        btn_frame.grid_rowconfigure(0, weight=1)
        btn_frame.grid_rowconfigure(1, weight=1)
        self.ok_btn = ctk.CTkButton(btn_frame, text=self.btn_lbls[0], font=self.chooser_font, command=self.on_ok, border_width=2, corner_radius=5)
        self.ok_btn.grid(row=0, column=0, sticky="nsew", padx=20, pady=10)
        self.cancel_btn = ctk.CTkButton(btn_frame, text=self.btn_lbls[1], font=self.chooser_font, command=self.on_cancel, border_width=2, corner_radius=5)
        self.cancel_btn.grid(row=0, column=1, sticky="nsew", padx=20, pady=10)
        self.set_initial_color(self.hex_color)
        self.update_canvas_picker()
        self.update_preview()
        ...
        self.geometry(f"{self.req_width}x{self.req_height}+{self.x}+{self.y}")
        self.deiconify()
        self.lift()
        self.update()
        self.grab_set()
        self.focus()
    def set_initial_color(self, hex_color):
        hex_color = hex_color.lstrip("#")
        r = int(hex_color[0:2], 16) / 255
        _r = int(r * 255)
        self.red_lbl.configure(text=f"Red = {str(_r)}")
        g = int(hex_color[2:4], 16) / 255
        _g = int(g * 255)
        self.green_lbl.configure(text=f"Green = {str(_g)}")
        b = int(hex_color[4:6], 16) / 255
        _b = int(b * 255)
        self.blue_lbl.configure(text=f"Blue = {str(_b)}")
        self.hue, self.sat, self.val = colorsys.rgb_to_hsv(r, g, b)
        self.hue_lbl.configure(text=f"Hue = {str(round(self.hue, 12))}")
        self.update_marker_position()
    def red_change(self, value):
        self.r = int(value)
        self.red_lbl.configure(text=f"Red = {str(self.r)}")
        self.update_from_rgb()
    def green_change(self, value):
        self.g = int(value)
        self.green_lbl.configure(text=f"Green = {str(self.g)}")
        self.update_from_rgb()
    def blue_change(self, value):
        self.b = int(value)
        self.blue_lbl.configure(text=f"Blue = {str(self.b)}")
        self.update_from_rgb()
    def hue_change(self, value):
        self.hue = float(value)
        self.hue_lbl.configure(text=f"Hue = {str(round(self.hue, 12))}")
        self.update_canvas_picker()
        self.update_preview()
    def picker_clicked(self, event):
        x, y = event.x, event.y
        w = self.canvas_picker.winfo_width()
        h = self.canvas_picker.winfo_height()
        x = max(0, min(w, x))
        y = max(0, min(h, y))
        if self.marker is not None:
            self.canvas_picker.delete(self.marker)
        r = self.marker_radius * self.scale
        self.marker = self.canvas_picker.create_oval(x - r, y - r, x + r, y + r, outline=self.marker_color, width=2)
        self.sat = min(max(event.x / w, 0), 1)
        self.val = 1 - min(max(event.y / h, 0), 1)
        self.update_preview()
    def update_marker_position(self):
        # Convert Hex Color To X, Y Coordinates On Color Picker Canvas
        hex_color = self.hex_color.lstrip("#")
        dec_color = int(hex_color, 16)
        if dec_color >= 12000000:
        ...
        wid = self.canvas_picker.winfo_width()
        hgt = self.canvas_picker.winfo_height()
        x = int(s * wid)
        y = int((1 - v) * hgt)
        _r = self.marker_radius * self.scale
        _x = max(0, min(wid, x))
        _y = max(0, min(hgt, y))
        if self.marker is not None:
            self.canvas_picker.delete(self.marker)
        self.marker = self.canvas_picker.create_oval(_x - _r, _y - _r, _x + _r, _y + _r,
                                                outline=self.marker_color, width=2)
        self.update_idletasks()
    def update_canvas_picker(self):
        # Get actual canvas size
        wid = self.canvas_picker.winfo_width()
        hgt = self.canvas_picker.winfo_height()
        if wid < 1 or hgt < 1:
            return  # Canvas not ready yet
        img = Image.new("RGB", (wid, hgt))
        for x in range(wid):
            for y in range(hgt):
                s = x / wid
                v = 1 - (y / hgt)
        ...
        self.canvas_picker.delete("all")
        self.canvas_picker.create_image(0, 0, anchor="nw", image=self.sv_img)
        self.update_marker_position()
        self.update_idletasks()
    def update_preview(self):
        r, g, b = colorsys.hsv_to_rgb(self.hue, self.sat, self.val)
        self.hex_color = "#{:02x}{:02x}{:02x}".format(int(r * 255), int(g * 255), int(b * 255))
        self.preview.configure(fg_color=self.hex_color)
        self.hex_label.configure(text=f"Hex Value = {self.hex_color}")
        self.r = int(r * 255)
        self.g = int(g * 255)
        self.b = int(b * 255)
    ...
    def handle_resize(self):
        try:    
            ...
            self.scale = min(scale_w, scale_h)  # Keep proportions
            self.canvas_picker.configure(width=self.canvas_width * self.scale, height=self.canvas_height * self.scale)
            self.preview.configure(width=self.canvas_width * self.scale, height=50 *self.scale)
            font_size1 = int(self.base_font_size * self.scale)
            self.chooser_font.configure(size=font_size1)
            font_size2 = int(self.hex_font_size * self.scale)
            self.hex_font.configure(size=font_size2)
            self.marker_radius = 10 * self.scale
            self.update_marker_position()
            self.update_idletasks()
        except Exception:
    ...
    def get(self):
        return self.result
class rwFontChooser(ctk.CTkToplevel):
    def __init__(self, parent, font_dict={}, text="Sample Text For Selected Font", **kwargs):
        ...
        fontfile=os.path.join(self.base_path, "Font_Families.json")
        if os.path.exists(fontfile):# Windows Font Family Names are stored in json file. Not reliant on tk families.
            for r in range(4):# Configure Rows For main_frame 
                self.grid_rowconfigure(r, weight=1)
            for c in range(11):# Configure Columns For main_frame 
                self.grid_columnconfigure(c, weight=1)
            with open(fontfile, 'r') as json_file:
                self.fonts = json.load(json_file)
                json_file.close()
            self.attributes("-topmost", False)
            self.font_dict=font_dict
            if Language == "English":
                title="Choose Calculator Display Font"
                self.sample_txt = text
            else:
                title=title="Elegir Fuente de Pantalla de la Calculadora"
                self.sample_txt = "Texto de Muestra para la Fuente Seleccionada"
            self.title(title)
            self.resizable(True, True)
            self.withdraw()
            self.after(250, self.wm_iconbitmap, app.ico_path)
            self.screen_width = ctypes.windll.user32.GetSystemMetrics(0)
            self.screen_height = ctypes.windll.user32.GetSystemMetrics(1)
            self.width = int(self.screen_width * 0.4084)
            self.height = int(self.screen_height * 0.47)
            self.factor = (Default_DPI / 96.0)  # 96 is standard DPI for 1 point
            self.base_font_size = int(-20 * self.factor) 
            self.bind("<Configure>", self.on_resize)
            self._resize_after_id = None
            ...
            try:
                ...
                if Language=="English":font_props=["Bold","Italic","Underline","Overstrike"]
                else:font_props=["Negrita","Cursiva","Subrayar","Sobreescribir"]
                self.var_family= ctk.StringVar(self)
                self.var_size = ctk.StringVar(self)
                # Column 1 Entry
                wid_1 = 469 /self.width
                hgt_1 = 30 / self.height
                x_1 = 10 / self.width
                y_1 = 10 / self.height
                # Column 2 Entry
                wid_2 = 100 / self.width
                x_2 = 489 / self.width
                # Column 3 Entry
                wid_3 = 175 / self.width
                x_3 = 599 / self.width
                self.entry_family = ctk.CTkEntry(self, placeholder_text="Enter Name", font=self.chooser_font,
                                            textvariable=self.var_family, corner_radius=10, state="normal")
                self.entry_family.place(relx=x_1, rely=y_1, relwidth=wid_1, relheight=hgt_1)
                self.entry_size = ctk.CTkEntry(self, placeholder_text="Enter Size", font=self.chooser_font, 
                                            textvariable=self.var_size, corner_radius=10, state="normal")
                self.entry_size.place(relx=x_2, rely=y_1, relwidth=wid_2, relheight=hgt_1)
                if Language == "English": 
                    txt = "Font Options"
                else: 
                    txt = "Opciones de Fuente"
                self.font_options = ctk.CTkEntry(self, placeholder_text=txt, font=self.chooser_font,  
                                            corner_radius=10, state="normal")
                self.font_options.place(relx=x_3, rely=y_1, relwidth=wid_3, relheight=hgt_1)
                self.font_options.insert(0, txt)
                y_2 = 50 / self.height
                hgt_2 = 220 / self.height
                self.list_family = CTkListbox(self, font=self.chooser_font,
                                            border_width=2, justify="left", corner_radius=10)
                self.list_family.place(relx=x_1, rely=y_2, relwidth=wid_1, relheight=hgt_2)
                self.sizes = ["%i" % i for i in (list(range(6, 41)) + list(range(42, 74, 2)))]
                self.list_size = CTkListbox(self, font=self.chooser_font, 
                                            border_width=2, justify="left", corner_radius=10)
                self.list_size.place(relx=x_2, rely=y_2, relwidth=wid_2, relheight=hgt_2)
                hgt_3 = 165 / self.height
                self.options_frame=ctk.CTkFrame(self,  border_width=2, corner_radius=10) 
                self.options_frame.place(relx=x_3, rely=y_2, relwidth=wid_3, relheight=hgt_3)
                x1 = 10 / 175
                y1 = 9 / 165
                y2 = 48 / 165
                y3 = 87 / 165
                ...
                b_bold = ctk.CTkCheckBox(self.options_frame, text=[font_props[0]], font=self.chooser_font, border_width=2,
                                        command=self.toggle_bold, variable=self.var_bold)
                b_bold.place(relx=x1, rely=y1, relwidth=wid, relheight=hgt)
                self.var_italic = ctk.BooleanVar(self, self.font_dict["slant"] == "italic")
                b_italic = ctk.CTkCheckBox(self.options_frame, text=[font_props[1]], font=self.chooser_font, border_width=2,
                                    command=self.toggle_italic, variable=self.var_italic)
                b_italic.place(relx=x1, rely=y2, relwidth=wid, relheight=hgt)
                self.var_underline = ctk.BooleanVar(self, self.font_dict["underline"])
                b_underline = ctk.CTkCheckBox(self.options_frame, text=[font_props[2]], font=self.chooser_font, border_width=2,
                                        command=self.toggle_underline, variable=self.var_underline)
                b_underline.place(relx=x1, rely=y3, relwidth=wid, relheight=hgt)
                self.var_overstrike = ctk.BooleanVar(self, self.font_dict["overstrike"])
                b_overstrike = ctk.CTkCheckBox(self.options_frame, text=[font_props[3]], font=self.chooser_font, border_width=2,
                                        variable=self.var_overstrike, command=self.toggle_overstrike)
                b_overstrike.place(relx=x1, rely=y4, relwidth=wid, relheight=hgt)
                y1 = 280 / self.height
                wid = (self.width - 20) / self.width
                hgt = 170 / self.height
                ...
                self.preview = ctk.CTkLabel(self, text=self.sample_txt, font=self.preview_font, anchor="w", justify="left",
                                            corner_radius=10, fg_color=parent.Display_bg.get(),text_color=parent.Display_fg.get())
                self.preview.place(relx=x_1, rely=y1, relwidth=wid, relheight=hgt)
                if Language == "English": 
                    txt1 = "OK"
                    txt2 = "Cancel" 
                else: 
                    txt1 = "Vale"
                    txt2 = "Cancelar"
                x_padding = (self.width - (2 * 175)) / 3
                x1 = x_padding / self.width
                x2 = (2 * x_padding + 175) / self.width
                wid_3 = 175 / self.width
                hgt_1 = 30 / self.height
                y = (self.height - 45) / self.height
                self.ok_btn = ctk.CTkButton(self, text=txt1, border_width=2, corner_radius=10, font=self.chooser_font,
                                border_color="black",  hover_color="#00ced1", command=self.ok)  
                self.ok_btn.place(relx=x1, rely=y, relwidth=wid_3, relheight=hgt_1)
                self.cancel_btn = ctk.CTkButton(self, text=txt2, border_width=2, corner_radius=10, font=self.chooser_font,
                                border_color="black", hover_color="#00ced1", command=self.quit) 
                self.cancel_btn.place(relx=x2, rely=y, relwidth=wid_3, relheight=hgt_1)
                for f in range(0, len(self.fonts)):# Populate The Families
                    self.list_family.insert(f, self.fonts[f])
                for s in range(0, len(self.sizes)):# Populate The Sizes    
                ...
                self.font_options.configure(state="readonly")
                try:
                ...
                self.preview.configure(wraplength=self.width - 20)
                self.update_idletasks()
    ...
    def handle_resize(self):
        try:    
            ...
            self.chooser_font.configure(size=font_size)
            self.update_idletasks()
        except Exception:
            return
    ...
    def ok(self):
        ...
        size_new = int(self.preview_font.cget("size") * (self.factor * -1)) 
    ...
    def quit(self):
        ...
        self.grab_release()
        self.destroy()    
class rwRatioCalculator(ctk.CTkToplevel):
    def __init__(self, app):
        super().__init__(app)
        try:
            app.withdraw()
            self.after(250, self.wm_iconbitmap, app.ico_path)
            if Language=="English":txt="Ratio / Porportion Calculator"
            else:txt="Calculadora de Proporciones / Ratios"
            self.title(f'{app.root_title}  {txt}')
            self.screen_width = ctypes.windll.user32.GetSystemMetrics(0)
            self.screen_height = ctypes.windll.user32.GetSystemMetrics(1)
            self.width = int(self.screen_width * 0.3)
            self.height = int(self.screen_height * 0.5)
            self.x = int((self.screen_width / 2) - (self.width / 2))
            self.y = int((self.screen_height / 2) - (self.height / 2))
            self.geometry(f"{self.width}x{self.height}+{self.x}+{self.y}")
            ...
            self.bind("<Configure>", self.on_resize)
            self._resize_after_id = None
            self.ratio_font=ctk.CTkFont(family='Arial', size=self.base_font_size, weight='normal', slant='italic')# Ratio Formulas
            self.formula=""
            ...
            if Language=="English":txt=['3 Ratio Values Must Be Present To Calculate','The 4th Ratio Value And Porportion Values.',
            ...
            kb_frame=ctk.CTkFrame(self, fg_color='#00FFFF', border_width=0, border_color="black", corner_radius=5) 
            ...
            for i, item in enumerate(keys): # Prevent Unwanted Keyboard Keys From Appearing In Display
                ...
                key_btn[i].pack(side='left',fill='both',expand=True,padx=2,pady=2)
    ...
    def handle_resize(self):
        try:    
            ...
            scale = min(scale_w, scale_h)  # Keep proportions
            font_size = int(self.base_font_size * scale)
            self.ratio_font.configure(size=font_size)
        except Exception:
            return
    ...
    def copy_display(self):
        ...
        x_pos = int((parent_wid / 2) - (popup_width / 2))
        y_pos = int((parent_hgt / 2) - (popup_height / 2))
        if Language=="English":msg="Copy Display Text To Clipboard"
    ...
    def calculate_ratio(self,a_1,a_2,b_1,b_2,a_3,b_3):
        try:
            ...
            if count < 3:
                ...
                msg=msg1+msg2+msg3
                rwDialog(parent=app, style="msgbox", title=title, prompt=msg, icon="info.png")
            ...
            if count == 4: # All ratio value present, examine for porportion values
                ...
                if Language=="English":ranges=['A Range','B Range']
...
class Calculus_Scroll(ctk.CTkFrame):
    def __init__(self, app):
        ...
        try:
            ...
            self.v_scrollbar.pack(side='right',fill='y',padx=(5, 5),pady=(20, 40))                                        
            self.h_scrollbar.pack(side='bottom',fill='x',expand=False, padx=(20, 20),pady=(5, 5))                                        
...
class Calculus(ctk.CTkToplevel):
    def __init__(self, app, funct):
        ...
        try:
            ...
            self.bind("<Configure>", self.on_resize)
            self._resize_after_id = None
            self.calculus_font = ctk.CTkFont(family="Arial", size=self.base_font_size)
            app.close_brackets('all')# (Just In Case) Close All Opened Brackets Before Preceding
            if Language=="English":txt="Expression"
            else:txt="Expresión"
            ...
            self.lbl_1 = ctk.CTkLabel(calculus.window, text=txt1, corner_radius=0, anchor="w",
                            fg_color=("#f3f6f4", "#f3f6f4"), text_color="black", font=self.calculus_font)
            ...
            if Language=="English":txt=[f'Expression Symbolized = {newstr}','With Respect To:','Derivative Work Sheet']
            ...
            r+=1
            self.cbo_lbl = ctk.CTkLabel(calculus.window, text=txt[1], corner_radius=0, anchor="w",
                            fg_color=("#f3f6f4", "#f3f6f4"), text_color="black", font=self.calculus_font)
            ...
            self.symbol_bx = ctk.CTkComboBox(calculus.window, values=self.used_symbols, button_color=("#00cccc","#00cccc"), 
                                            ...
                                            dropdown_text_color=("black", "black"), font=self.calculus_font, dropdown_font=self.calculus_font) 
            ...
            if self.funct=='differentiate':
                ...
                self.multi_lbl = ctk.CTkLabel(calculus.window, text=txt, corner_radius=0, anchor="w", 
                                fg_color=("#f3f6f4", "#f3f6f4"), text_color="black", font=self.calculus_font)
                ...
                if Language=="English":txt='Integral Work Sheet'
            ...
            for i in range(0,6):
                r+=1
                self.lbl_4.append([i])
                self.lbl_4[i] = ctk.CTkLabel(calculus.window, text="", corner_radius=0, anchor="w", 
                                fg_color=("#f3f6f4", "#f3f6f4"), text_color="black", font=self.calculus_font)
    ...
    def handle_resize(self):
        try:    
            ...
            scale = min(scale_w, scale_h)  # Keep proportions
            font_size = int(self.base_font_size * scale)
            self.calculus_font.configure(size=font_size)
        except Exception:
            return
    def do_integral(self, event):
        try:
            ...
            if Language=="English":txt=['Equation','"Answer','Invalid Entry!','Get Integral']
    ...
    def calculus_destroy(self):# X Icon Was Clicked
        ...
        app.focus_force()
        app.update()
class rwTimedPopup(ctk.CTkLabel): # The Popup Is Centered On The Parent Widget. 
    def __init__(self, parent, text, text_color, fg_color, font_size, delay_time): 
        ...
        self.base_font_size = int(font_size * factor) 
        self.delay_time=delay_time
        try:# If App is Resized, Resize rwTimedPopup Font
            width = app.winfo_width()
            scale_w = width / app.width
            height = app.winfo_height()
            scale_h = height / app.height
            scale = min(scale_w, scale_h)  # Keep proportions
            ...
            self.popup_font = ctk.CTkFont(family="Arial", size=self.base_font_size, weight="normal", slant="roman")
            pass   
        try:
            self.update_idletasks()
            ...
            parent_wid=self.parent.winfo_width()#/ scale_factor
            parent_hgt=self.parent.winfo_height()# / scale_factor
            ...
            self.destroy()    
class rwPopupMenu(ctk.CTkToplevel):
    def __init__(self, parent, caption_list, command_list, font_size, x_pos, y_pos, y_ref=None):
        ...
        self.scale = 0
        self.factor = (Default_DPI / 96.0)  # 96 is standard DPI for 1 point
        self.base_font_size = int(font_size * self.factor) 
        try:# If App is Resized, Resize rwTimedPopup Font
            width = app.winfo_width()
            scale_w = width / app.width
            height = app.winfo_height()
            scale_h = height / app.height
            self.scale = min(scale_w, scale_h)  # Keep proportions
            self.font_size = int(self.base_font_size * self.scale)
            self.menu_font = ctk.CTkFont(family="Arial", size=self.font_size, weight="normal", slant="roman")
        except Exception as e:
            self.menu_font = ctk.CTkFont(family="Arial", size=self.base_font_size, weight="normal", slant="roman")
            pass   
        ...
        try:
            ...
            self.req_height = int(app.winfo_height() * 0.9)
            self.req_width = self.menu_font.measure(self.text)
            self.x_pos = int(x_pos - ((self.req_width * self.scale_factor) * 0.5))
...
class rwDialog(ctk.CTkToplevel):
    def __init__(self, parent, style: Literal["msgbox", "entry"], title, prompt, choices=None, 
        ...
        self.ico_path=os.path.join(pgm_path, 'pycal.ico')
        ...
        self.factor = (Default_DPI / 96.0)  # 96 is standard DPI for 1 point
        self.base_font_size = int(-22 * self.factor)
        self.bind("<Configure>", self.on_resize)
        self._resize_after_id = None
        ...
        label = ctk.CTkLabel(self, text=prompt, font=self.rwDialog_font, anchor="w", justify="left")# Widgets for the dialog
        label.pack(pady=(10, 10))
        if self.style == "entry": 
            if self.choices is not None:
                if type(self.choices)==list:# List
                    pad_x = int(self.width * 0.15)
                    self.combobox = ctk.CTkComboBox(self, values=self.choices, font=self.rwDialog_font, command=self.on_select) 
                    self.combobox.pack(fill= "x", expand=True, padx=(pad_x, pad_x))
...
class Calculator(ctk.CTk):
    def __init__(self):
        ...
        self.Temperature=ctk.StringVar(value='')
        self.Length=ctk.StringVar(value='')
        self.Volume=ctk.StringVar(value='')
        self.Area=ctk.StringVar(value='')
        ...
        self.Length_List = ['in. → mm','in. → cm', 'in. → m','in. → km','in. → ft','in. → yds','in. → mi','in. → au',
                            'ft → mm','ft → cm','ft → m','ft → km','ft → in.','ft → yds','ft → mi','ft → au',
                            'yds → mm','yds → cm','yds → m','yds → km','yds → in.','yds → ft','yds → mi','yds → au',
                            'mi → mm','mi → cm','mi → m','mi → km','mi → in.','mi → ft','mi → yds','mi → au', 'mi → ly',
                            'mm → in.','mm → ft','mm → yds','mm → mi','mm → au',
                            'cm → in.','cm → ft','cm → yds','cm → mi','cm → au',
                            'm → in.','m → ft','m → yds','m → mi','m → au',
                            'km → in.','km → ft','km → yds','km → mi','km → au','km → ly',
                            'au → in.','au → ft','au → yds','au → mi','au → mm','au → cm','au → m','au → km','au → ly',
                            'ly → mi', 'ly → km', 'ly → au']
        self.Length_Math = ['* 25.400','* 2.5400', '* 0.025400','* 0.000025400',' / 12.0',' / 36.0',' / 63360.0',' / 5889679948818.9',
                            ...
                            ' / 0.000000621371',' / 0.00000621371 ',' / 0.000621371',' / 0.621371',' * 63360.0',' * 5280.0',' * 1760.0',' / 92955807.267433', '/ 5878625370000.0', 
                            '/ 25.400','0.0032808399',' * 0.00109361',' * 0.000000621371',' / 149597870700000.0',
                            '/ 2.5400',' * 0.032808398950131',' * 0.0109361329833771',' * 0.00000621371',' / 14959787070000.0',
                            '/ 0.025400',' * 3.2808398950131',' * 1.09361329833771',' * 0.000621371',' / 149597870700.0',
                            '/ 0.000025400',' * 3280.8398950131',' * 1093.61329833771',' * 0.621371',' / 149597870.7', '/ 9460700000000.0',
                            ' * 5889679948818.9',' * 490806662372.05',' * 163602220790.68',' *  92955807.267433',' * 149597870700000.0',' * 14959787070000.0',' * 149597870700.0',' * 149597870.7', '/ 63241.0770',
                            '* 5878625370000.0', '* 9460700000000.0', '* 63241.0770']
        self.Volume_List = ['tsp → tbsp', 'tsp → fl oz', 'tsp → cup', 'tsp → mL',
                            'tbsp → tsp', 'tbsp → fl oz', 'tbsp → cup', 'tbsp → mL',
                            'fl oz → tsp', 'fl oz → tbsp', 'fl oz → cup', 'fl oz → pt', 'fl oz → qt', 'fl oz → gal', 'fl oz → cm³', 'fl oz → m³', 'fl oz → mL', 'fl oz → L',  
                            'cup → tsp', 'cup → tbsp', 'cup → fl oz', 'cup → pt', 'cup → qt', 'cup → gal', 'cup → mL', 'cup → cm³', 'cup → m³', 'cup → L',
                            'pt → fl oz', 'pt → cup', 'pt → qt', 'pt → gal', 'pt → mL', 'pt → cm³', 'pt → m³', 'pt → L', 'pt → in³', 'pt → ft³',
                            'qt → fl oz', 'qt → cup', 'qt → pt', 'qt → gal', 'qt → mL', 'qt → cm³', 'qt → m³', 'qt → L', 'qt → in³', 'qt → ft³',
                            'gal → fl oz', 'gal → cup', 'gal → pt', 'gal → qt', 'gal → mL', 'gal → L', 'gal → cm³', 'gal → m³', 'gal → in³', 'gal → ft³', 'gal → yd³',
                            'in³ → mL', 'in³ → cm³', 'in³ → m³', 'in³ → L', 'in³ → ft³', 'in³ → fl oz', 'in³ → cup', 'in³ → pt', 'in³ → qt', 'in³ → gal',
                            'ft³ → mL', 'ft³ → cm³', 'ft³ → m³', 'ft³ → L', 'ft³ → in³', 'ft³ → fl oz','ft³ → cup', 'ft³ → pt', 'ft³ → qt', 'ft³ → gal', 'ft³ → yd³',
                            'mL → tsp', 'mL → tbsp', 'mL → fl oz', 'mL → cup', 'mL → pt', 'mL → qt', 'mL → gal', 'mL → L', 'mL → cm³', 'mL → m³', 'mL → in³', 'mL → ft³',
                            'cm³ → mL', 'cm³ → m³', 'cm³ → L', 'cm³ → in³', 'cm³ → ft³', 'cm³ → fl oz', 'cm³ → cup', 'cm³ → pt', 'cm³ → qt', 'cm³ → gal',
                            'm³ → cm³', 'm³ → mL','m³ → L', 'm³ → in³', 'm³ → ft³', 'm³ → fl oz', 'm³ → cup', 'm³ → pt', 'm³ → qt', 'm³ → gal', 'm³ → yd³',
                            'L → mL', 'L → cm³', 'L → m³', 'L → in³', 'L → ft³', 'L → fl oz', 'L → cup', 'L → pt', 'L → qt', 'L → gal', 'L → yd³',
                            'yd³ → gal', 'yd³ → L', 'yd³ → ft³', 'yd³ → m³']
        self.Volume_Math = ['/ 3.0','/ 6.0', '/ 48.0', '* 4.92892159',
                            '* 3.0', '* 0.500', '* (1 / 16)', '* 14.78676477',
                            '* 6.0', '/ 0.500', '/ 8.0', '/ 16', '/ 32', '/ 128', '/ 0.0338140227', '* 0.00002957', '* 29.5735', '* 0.0295735', 
                            '* 48.0', '/ (1 / 16)', '* 8.0', '* 0.5', '* 0.25', '* (1 / 16)', '* 236.588236', '* 236.588236', '* 0.000236588236', '* 0.236588236',
                            '* 16', '/ 0.5', '/ 2.0', '/ 8.0', '* 473.17647', '* 473.17647', '* 0.00047317647', '* 0.47317647', '* 28.875', '* 0.01671007',
                            '* 32', '/ 0.25', '* 2.0', '/ 4.0', '* 946.352946', '* 946.352946', '* 0.000946352946', '* 0.946352946', '* 57.75', '* 0.03342014',
                            '* 128', '/ (1 / 16)', '* 8.0', '* 4.0', '* 3785.41178', '* 3.78541178', '* 3785.41178', '* 0.00378541178', '* 231', '* 0.13368056', '* 0.00495113',
                            '* 16.387064', '* 16.387064', '* 0.000016387064', '* 0.016387064', '* 0.0005787', '* 0.554112554', '* 0.0692641', '/ 28.875', '/ 57.75', '/ 231',
                            '* 28316.8466', '* 28316.8466', '* 0.02831685', '* 28.3168466', '/ 0.0005787', '* 957.506494','* 119.688312', '/ 0.01671007', '/0.03342014', '/ 0.13368056', '* 0.03703704',
                            '/ 4.92892159', '/ 14.78676477', '/ 29.5735', '/ 236.588236', '/ 473.176', '/ 946.352946', '/ 3785.41178', '* 0.001', '* 1,0', '* 0.000001', '/ 16.387064', '/ 28316.8466',
                            '/ 1.0', '* 0.000001', '* 0.001', '/ 16.387064', '/ 28316.8466', '* 0.0338140227', '/ 236.588236', '/ 473.1765', '/ 946.352946', '/ 3785.41178',
                            '/ 0.000001', '/ 0.000001','* 1000', '/ 0.000016387064', '/ 0.02831685', '/ 0.00002957', '/ 0.000236588236', '/ 0.0004731765', '/ 0.000946352946', '/ 0.00378541178', '* 1.30795062',
                            '/ 0.001', '/ 0.001', '/ 1000', '/ 0.016387064', '/ 28.3168466', '/ 0.0295735', '/ 0.236588236', '/ 0.47317647', '/ 0.946352946', '/ 3.78541178', '* 0.00130795',
                            '/ 0.00495113', '/ 0.00130795', '/ 0.03703704', '/ 1.30795062']
        self.Area_List = ['in² → ft²', 'in² → yd²', 'in² → mi²', 'in² → mm²', 'in² → cm²', 'in² → m²', 'in² → km²', 'in² → ac', 'in² → ha', 'in² → sec',
                          'ft² → in²', 'ft² → yd²', 'ft² → mi²', 'ft² → mm²', 'ft² → cm²', 'ft² → m²', 'ft² → km²', 'ft² → ac', 'ft² → ha', 'ft² → sec',
                          'yd² → in²', 'yd² → ft²', 'yd² → mi²', 'yd² →  mm²', 'yd² → cm²', 'yd² → m²', 'yd² → km²', 'yd² → ac', 'yd² → ha', 'yd² → sec',
                          'mi² → in²', 'mi² → ft²', 'mi² → yd²', 'mi² → mm²', 'mi² → cm²', 'mi² → m²', 'mi² → km²', 'mi² → ac', 'mi² → ha', 'mi² → sec',
                          'mm² → in²', 'mm² → ft²', 'mm² → yd²', 'mm² → mi²', 'mm² → cm²', 'mm² → m²', 'mm² → km²', 'mm² → ac', 'mm² → ha', 'mm² → sec',
                          'cm² → in²', 'cm² → ft²', 'cm² → yd²', 'cm² → mi²', 'cm² → mm²', 'cm² → m²', 'cm² → km²', 'cm² → ac', 'cm² → ha', 'cm² → sec',
                          'm² → in²', 'm² → ft²', 'm² → yd²', 'm² → mi²', 'm² → mm²', 'm² → cm²', 'm² → km²', 'm² → ac', 'm² → ha', 'm² → sec',
                          'km² → in²', 'km² → ft²', 'km² → yd²', 'km² → mi²', 'km² → mm²', 'km² → cm²', 'km² → m²', 'km² → ac', 'km² → ha', 'km² → sec',
                          'ac → in²', 'ac → ft²', 'ac → yd²', 'ac → mi²', 'ac → mm²', 'ac → cm²', 'ac → m²', 'ac → km²', 'ac → ha', 'ac → sec',
                          'ha → in²', 'ha → ft²', 'ha → yd²', 'ha → mi²', 'ha → mm²', 'ha → cm²', 'ha → m²', 'ha → km²', 'ha → ac', 'ha → sec',
                          'sec → in²', 'sec → ft²', 'sec → yd²', 'sec → mi²', 'sec → mm²', 'sec → cm²', 'sec → m²', 'sec → km²', 'sec → ac', 'sec → ha']

        self.Area_Math = ['/ 144.0', '/ 1296.0', '/ 27878400.0', '* 645.16', '* 6.4516', '/ 1550.0031', '/ 1550003100.0', '/ 6272640.0', '/ 155000310.0', '/ 4014489600.0',
                          '* 144.0', '/ 9', '/ 27878400.0', '* 92903.04', '* 929.0304', '/ 10.763910417', '/ 10763910.417', '/ 43560.0', '/ 107639.10417', '/ 27878400.0',
                          '* 1296.0', '* 9', ' / 3097601.33329', '* 836127.36', '* 8361.2736', '/ 1.1959900463', '/ 1195990.0463', '/ 4840.0', '/ 11959.9005', '/ 3097600.0',
                          '* 27878400.0', '* 27878400.0', '* 3097601.33329', '* 2589988110336.0', '* 25899881103.36', '* 2589988.110336', '* 2.58998811', '* 640.0', '* 258.9988110336', '/ 1.0',
                          '/ 645.16', '/ 92903.04', '/ 836127.36', '/ 2589988110336.0', '/ 100.0', '/ 1000000.0', '/ 1000000000000.0', '/ 4046856.4224', '/ 10000000000.0', '/ 2589988110336.0',
                          '/ 6.4516', '/ 929.0304', '/ 8361.2736', '/ 25899881103.36', '* 100.0', '/ 10000.0', '/ 10000000000.0', '/ 40468564.224', '/ 100000000.0', '/ 25899881103.36',
                          '* 1550.0031', '* 10.763910417', '* 1.1959900463', '/ 2589988.110336', '* 1000000.0', '* 10000.0', '/ 1000000.0', '/ 4046.8564224', '/ 10000.0', '/ 2589988.110336',
                          '* 1550003100.0', '* 10763910.417', '* 1195990.0463', '/ 2.589988110336', '* 1000000000000.0', '* 10000000000.0', '* 1000000.0', '/ 0.0040468564224', '/ 0.01', '/ 2.589988110336',
                          '* 6272640.0', '* 43560.0', '* 4840.0', '/ 640.0', '* 4046856.4224', '* 40468564.224', '* 4046.8564224', '* 0.0040468564224', '/ 2.47105381', '/ 640',
                          '* 155000310.0', '* 107639.10417', '* 11959.9005', '/ 258.9988110336', '* 10000000000.0', '* 100000000.0', '* 10000.0', '* 0.01', '* 2.47105381', '/ 258.9988110336',
                          '* 4014489600.0', '* 27878400.0', '* 3097600.0', '* 0.1', '* 2589988110336.0', '* 25899881103.36', '* 2589988.110336', '* 2.589988110336', '* 640', '* 258.9988110336']
        self.Temp_List = ['°F → °C', '°F → °K','°F → °R', '°C → °F', '°C → °K', '°C → °R', '°K → °F','°K → °C','°K → °R', '°R → °F','°R → °C','°R → °K']
        data = json.load(open("Config.json", "r+"))
        self.Display_Font=ctk.CTkFont()
        global Default_DPI 
        if len(data)==1:# Only Theme Exist
            Default_DPI = self.winfo_fpixels('1i')
            self.width = int(self.screen_width * 0.4)
            self.height = int(self.screen_height * 0.35)
            self.x = int((self.screen_width / 2) - ((self.width) / 2))    
            self.y=((self.screen_height / 2) - (self.height / 2))
            self.geometry_str = f"{self.width}x{self.height}+{self.x}+{self.y}"
            self.size_normal = -24
            self.Display_Font=ctk.CTkFont(family='Arial', size=self.size_normal, weight='normal', slant='italic')# Display Only
        else:
            Default_DPI = data["1"]
            ...
            self.size_normal = new_font['size']
        std_sizes = [-12, -14, -16, -22, -24]
        self.base_font_sizes = []
        factor = (Default_DPI / 96.0)  # 96 is standard DPI for 1 point
        for f in range(len(std_sizes)):
            self.base_font_sizes.append(int(std_sizes[f] * factor))  # adjust for high‑DPI        
        self.formula_font=ctk.CTkFont(family='Arial', size=self.base_font_sizes[0], weight='normal', slant='roman')# Ratio Formulas
        self.dialog_font=ctk.CTkFont(family='Arial', size=self.base_font_sizes[0], weight='normal', slant='italic')# Dialogs
        self.conv_font=ctk.CTkFont(family='Arial', size=self.base_font_sizes[0], weight='bold', slant='italic')# Dialogs
        self.base_font=ctk.CTkFont(family='Arial', size=self.base_font_sizes[1], weight='bold', slant='roman')# Base Buttons
        self.temp_font=ctk.CTkFont(family='Arial', size=self.base_font_sizes[1], weight='bold', slant='italic')# F-C, C-F, Ratio Buttons
        self.memory_font=ctk.CTkFont(family='Arial', size=self.base_font_sizes[2], weight='normal', slant='italic')# Memory, mod, exp Buttons
        self.integral_font=ctk.CTkFont(family='Arial', size=self.base_font_sizes[2], weight='bold', slant='italic')# Integrate Button
        self.symbols_font=ctk.CTkFont(family='Arial', size=self.base_font_sizes[3], weight='normal', slant='italic')# Numeric,math symbols, CM Buttons
        self.bracket_font=ctk.CTkFont(family='Arial', size=self.base_font_sizes[3], weight='normal', slant='roman')# () Buttons
        self.operator_font=ctk.CTkFont(family='Arial', size=self.base_font_sizes[4], weight='normal', slant='roman')# Math operators, sign Buttons
        self.size_zoomed = None
        global Language
        Language = ""
        if len(data)==1 or data["0"] == "" :# Only Theme Exist Or No Language
            ...
            while Language == "": 
                ...
                if lang.result is not None:
                    ...
                    Language = ""
        else:Language = data["0"]            
        self.user=os.getlogin()
        if Language=="English":txt=[f"{self.user}'s Calculator "," Right Click Display For Options"]
        else:txt=[f"{self.user}'s Calculadora "," Haga Clic Derecho en la Pantalla para Opciones"]
        self.root_title=txt[0]
        options=txt[1]
        self.title(self.root_title + options.rjust(10+len(options)))
        ...
        if Language=="English":text=['Binary','Decimal','Hexadecimal','Octal']
        else:text=['Binario','Decimal','Hexadecimal','Octal']
        wid=[0.19,0.21,0.32,0.185]
        x=[0.02,0.23,0.455,0.793]
        hgt=0.75
        y=0.15
        relwidth=0.479 * self.width
        relheight=0.1 * self.height
        self.base_frame=ctk.CTkFrame(self, border_width=2, corner_radius=10, width=relwidth, height=relheight) 
        ...
        for num in range(0,len(text)):
            ...
            self.base_btn[num] = ctk.CTkButton(self.base_frame, text=text[num], border_width=2, corner_radius=5, font=self.base_font, anchor="center",
                            fg_color=("navy", "navy"), hover_color="#00ced1", text_color="#ffffff",
        ...
        wid=[0.29,0.29,0.305]
        x=[0.03,0.35,0.665]
        self.unit_frame=ctk.CTkFrame(self, border_width=2, corner_radius=10) 
        ...
        for num in range(0,len(text)):
            ...
            self.unit_btn[num] = ctk.CTkButton(self.unit_frame, text=text[num], border_width=2, corner_radius=5, font=self.base_font, anchor="center",
                            fg_color=("#004d00", "#004d00"), hover_color="#00ced1", text_color="#ffffff",
        ...
        self.clrmem_btn = ctk.CTkButton(self, text='CM', border_width=2, corner_radius=5, font=self.symbols_font, anchor="center",
                            fg_color=("#ffff99", "#ffff99"), hover_color="#00ced1", text_color="maroon",
        ...
        self.pad_fra=ctk.CTkFrame(self, border_width=2, corner_radius=10) 
        ...
        x=[0.01,0.11,0.21,0.31,0.41,0.51]
        wid=0.09   
        ...
        for num in range(10,16):
            ...
            self.num_btn[num].place(relx=x[num-10], rely=y, relwidth=wid, relheight=0.12)
            self.num_btn[num].bind("<Button-1>", self.numeric_clicked) # Bind all the number keys with the callback function
        x=0.184
        y=0.015
        wid=0.088
        ...
        for num in range(0,len(self.operators)):
            ...
            self.operator_btn[num] = ctk.CTkButton(self.pad_fra, text=self.operators[num], anchor="c", border_width=2, corner_radius=5, font=self.operator_font,
                            fg_color=("white", "white"), hover_color="#00ced1", text_color="navy",
        ...
        self.equal_btn = ctk.CTkButton(self.pad_fra, text=' = ', anchor="c", border_width=2, corner_radius=5, font=self.operator_font,
                        fg_color=("#14fafa", "#14fafa"), hover_color="yellow", text_color="navy",
        ...
        for num in range(0,len(text)):
            ...
            self.log_btn[num] = ctk.CTkButton(self.pad_fra, text=text[num], anchor="c", border_width=2, corner_radius=5, font=self.base_font,
                            fg_color=("purple", "purple"), hover_color="#00ced1", text_color="whitesmoke",
        ...
        self.sign_btn = ctk.CTkButton(self.pad_fra, text=chr(177), anchor="n", border_width=2, corner_radius=5, font=self.operator_font,
                        fg_color=("navy", "navy"), hover_color="#00ced1", text_color="whitesmoke",
        ...
        for num in range(0,len(text)):
            ...
            self.trig_btn[num] = ctk.CTkButton(self.pad_fra, text=text[num], anchor="c", border_width=2, corner_radius=5, font=self.base_font,
                            fg_color=("#004d00", "#004d00"), hover_color="#00ced1", text_color="white",
        ...
        self.open_btn = ctk.CTkButton(self.pad_fra, text='('+chr(8304), anchor="n", border_width=2, corner_radius=5, font=self.operator_font,
                        fg_color=("white", "white"), hover_color="#00ced1", text_color="black")  
        ...
        for num in range(0,len(text)): # Function Buttons Column 1
            ...
            self.func_btn[num] = ctk.CTkButton(self.pad_fra, text=text[num], anchor="c", border_width=2, corner_radius=5, font=self.symbols_font,
                            hover_color="#00ced1", text_color=("black","white"),
        ...
        self.clr = ctk.CTkButton(self.pad_fra, text="C", anchor="c", border_width=2, corner_radius=5, font=self.symbols_font,
                    hover_color="#00ced1", text_color=("#0c012e","#ffffff"), fg_color=("#ffffff","#0c012e"),
        ...
        self.ce=ctk.CTkButton(self.pad_fra, text='CE', anchor="c", border_width=2, corner_radius=5, font=self.symbols_font, 
                        hover_color="#00ced1", text_color=("#0c012e","#ffffff"), fg_color=("#ffffff","#0c012e")) # Clear Entry Button 
        ...
        self.temp_btn = ctk.CTkButton(self.pad_fra, anchor="c", border_width=2, corner_radius=5, font=self.conv_font,
                    fg_color=("#14fafa", "#14fafa"), hover_color="#00ced1", text_color="black",
                    command=lambda: self.temp_clicked())
        self.temp_btn.place(relx=0.01, rely=0.855, relwidth=0.12, relheight=0.12)
        self.length_btn = ctk.CTkButton(self.pad_fra, anchor="c", border_width=2, corner_radius=5, font=self.conv_font,
                    fg_color=("#14fafa", "#14fafa"), hover_color="#00ced1", text_color="black",
                    command=lambda: self.length_clicked())  
        self.length_btn.place(relx=0.135, rely=0.855, relwidth=0.12, relheight=0.12,)
        self.volume_btn = ctk.CTkButton(self.pad_fra, anchor="c", border_width=2, corner_radius=5, font=self.conv_font,
                    fg_color=("#14fafa", "#14fafa"), hover_color="#00ced1", text_color="black",
                    command=lambda: self.volume_clicked())  
        self.volume_btn.place(relx=0.26, rely=0.855, relwidth=0.12, relheight=0.12)
        self.area_btn = ctk.CTkButton(self.pad_fra, anchor="c", border_width=2, corner_radius=5, font=self.conv_font,
                    fg_color=("#14fafa", "#14fafa"), hover_color="#00ced1", text_color="black",  
                    command=lambda: self.area_clicked())  
        self.area_btn.place(relx=0.385, rely=0.855, relwidth=0.12, relheight=0.12)
        self.select_btn = ctk.CTkButton(self.pad_fra, anchor="c", border_width=2, corner_radius=5, font=self.integral_font,
                    fg_color=("#ffcc99", "#ffcc99"), hover_color="#00ced1", text_color="black", command=lambda: self.show_ratio_calculus_menu()) 
        self.select_btn.place(relx=0.51, rely=0.855, relwidth=0.09, relheight=0.12)
        text=[]
        self.set_defaults()
    ...
    def handle_resize(self):
        try:    
            ...
            self.scale = min(scale_w, scale_h)  # Keep proportions
            new_sizes = []
            for f in range(len(self.base_font_sizes)):
                font_size = int(self.base_font_sizes[f] * self.scale)
                new_sizes.append(font_size)
            self.formula_font.configure(size=new_sizes[0])# Ratio Formulas
            self.dialog_font.configure(size=new_sizes[0])# Dialogs
            self.conv_font.configure(size=new_sizes[0])# Dialogs
            self.base_font.configure(size=new_sizes[1])# Base Buttons
            self.temp_font.configure(size=new_sizes[1])# F-C, C-F, Ratio Buttons
            self.memory_font.configure(size=new_sizes[2])# Memory, mod, exp Buttons
            self.integral_font.configure(size=new_sizes[2])# Integrate Button
            self.symbols_font.configure(size=new_sizes[3])# Numeric,math symbols, CM Buttons
            self.bracket_font.configure(size=new_sizes[3])# () Buttons
            self.operator_font.configure(size=new_sizes[4])# Math operators, sign Buttons
            state = self.state()
            if state == 'normal':
                self.Display_Font.configure(size=self.size_normal)
            elif state == 'zoomed':    
                self.size_zoomed = int(self.size_normal * self.scale) 
                self.Display_Font.configure(size=self.size_zoomed)
            else:
                self.size_normal = int(self.size_normal * self.scale)
                self.Display_Font.configure(size=self.size_normal)
        except Exception:
            return
    ...
    def get_taskbar_height(self):
        try:
            ...
            return None
    def do_calculus(self, funct):
        dsp_txt =  self.Display_Text.get("1.0", "end").replace('\n', '')
        if not app.Disp_List or dsp_txt.isnumeric():
            if Language == 'English':
                txt = 'An Expression Must Be Present Here First!'
            else:
                txt = '¡Primero Debe Aparecer una Expresión Aquí!'    
            rwTimedPopup(self.Display_Text, text=txt, text_color="#000000", 
                            fg_color="#14fafa", font_size=-24, delay_time=2000)
            return
    ...
    def precision(self, which): # Calculator Precision Configuration
        ...
        if which=='dp':
            dpp=rwDialog(self, title=titles[0], style="entry", prompt=messages[0], choices=None, init_val=mp.dps, min_val=1, max_val=500, icon="setup.png")
            ...
            rnd=rwDialog(self, title=titles[1], style="entry", prompt=messages[1], choices=None, init_val=self.Round.get(), min_val=0, max_val=500, icon="setup.png")
            if rnd.result is not None:
                while int(rnd.result) < 0 or int(rnd.result) > 500: 
                    rnd=rwDialog(self, title=titles[1], style="entry", prompt=messages[1], choices=None, init_val=self.Round.get(), min_val=0, max_val=500, icon="setup.png")
                self.Round.set(int(rnd.result))
        elif which=='exp':        
            snp=rwDialog(self, title=titles[2], style="entry", prompt=messages[2], choices=None, init_val=self.Exp_Precision.get(), min_val=1, max_val=500, icon="setup.png")
            ...
            enp=rwDialog(self, title=titles[3], style="entry", prompt=messages[3], choices=None, init_val=self.Exp_Digits.get(), min_val=1, max_val=4, icon="setup.png")
            if enp.result is not None:
                while int(enp.result) < 1 or int(enp.result) > 4: 
                    enp=rwDialog(self, title=titles[3], style="entry", prompt=messages[3], choices=None, init_val=self.Exp_Digits.get(), min_val=1, max_val=4, icon="setup.png")
                self.Exp_Digits.set(int(enp.result))
        elif which=='bit_size':
            bbs=rwDialog(self, title=titles[4], style="entry", prompt=messages[4], choices=['8','16','32','64','128','256','512'], init_val=self.Bit_Size.get(), min_val=None, max_val=None, icon="setup.png")
    ...
    def choose_font(self): # Calculator Display Font
        try:
            self.popup_menu.withdraw()
            app.update()
            if Language=="English":txt=["Sample text for selected font","Choose Calculator Display Font"]
            ...
            chooser = rwFontChooser(self, font_dict, text=sample)
            chooser.wait_window()
            self.deiconify()
            if chooser.new_font != '': 
                new_font_dict=chooser.new_font
                family_new=new_font_dict["family"]
                size_new=new_font_dict["size"] * -1
                weight_new=new_font_dict["weight"]
                slant_new=new_font_dict["slant"]
                self.size_normal = size_new
                self.size_zoomed = int(size_new * self.scale) # Adjust this multiplier as needed
    ...
    def choose_theme(self, theme):
        if Language=="English":
            msg = "Select The Desired Color Theme."
        else:
            msg = "Seleccione el Tema de Color Deseado."
        theme=rwDialog(self, title=" < Set Color Theme >", style="entry", prompt=msg, choices=["System", "Dark", "Light"], init_val=self.Theme.get(), icon="setup.png")
    ...
    def choose_color(self, which):
        self.popup_menu.withdraw()
        if Language == "English":
            txt_0 = "Choose Calculator Display Background Color"
            txt_1 = "Choose Calculator Display Text Color"
        else:
            txt_0 = "Elegir Color de Fondo de la Pantalla"
            txt_1 = "Elegir Color del Texto de Vvisualización"
        if which == 'fg_color':
            title = txt_0
            color = self.Display_bg.get()
        else:
            title = txt_1
            color = self.Display_fg.get()
        picker = rwColorPicker(master=self, init_color=color, title=title)
        picker.wait_window()
        color_code = picker.get()
        if color_code:
            if which=='fg_color':
    ...
    def log_clicked(self, text):
        ...
        try:       
            ...
            rwDialog(parent=app, style="msgbox", title=title, prompt=msg, icon="cancel.png")
            return 'break'
    def select_length(self, event):
        commands=[]
        for c in range(len(self.Length_List)):
            commands.append(c)
            commands[c] = lambda arg=self.Length_List[c]: self.update_length_btn(arg)
        x = self.winfo_pointerx()
        y = self.winfo_y()
        self.popup_menu = rwPopupMenu(self, caption_list=self.Length_List ,command_list=commands, font_size=-20, x_pos=x, y_pos=y)
        self.wait_window(self.popup_menu)
        self.popup_menu.grab_release()
        self.popup_menu.destroy()
        if not self.Disp_List:return
        else:self.length_clicked()
    def update_length_btn(self, txt):
        txt1 = f'{txt} ='
        self.length_btn.configure(text=txt1)       
        self.Length.set(txt)
    def length_clicked(self):
        base=self.Base.get()
        if base!='Decimal':return
        txt=self.Length.get()
        if self.length_btn.cget('text') =='Length' or self.length_btn.cget('text') =='Longitud':        
            self.select_length(self.Length.get())
        else:
            if not self.Disp_List:return
            try:
                if self.Answer_Present.get():
                ...
                self.reverse_value('expr',True)
                if txt in self.Length_List:
                    index = self.Length_List.index(txt)
                    display_txt=txt.split(" → ")[0]
                    self.disp_update(f' {display_txt}')
                    self.expr_update('(')
                    self.return_value('expr',True,True) 
                    self.expr_update(self.Length_Math[index])
                    self.expr_update(')')
                self.equal_clicked(txt)
                if Language == "English": txt = "Length"
                else: txt = "Longitud"
                self.length_btn.configure(text=txt)
            except Exception as e:
                if Language=="English":
    ...
    def select_temps(self, event):
        ...
        x = self.winfo_pointerx()
        y = self.winfo_y()
        self.popup_menu = rwPopupMenu(self, caption_list=self.Temp_List ,command_list=commands, font_size=-20, x_pos=x, y_pos=y)
        self.wait_window(self.popup_menu)
        self.popup_menu.grab_release()
        self.popup_menu.destroy()
        if not self.Disp_List:return
    ...
    def update_temp_btn(self, txt):
        txt1 = f'{txt} ='
        self.temp_btn.configure(text=txt1)       
        self.Temperature.set(txt)
    def temp_clicked(self):
        base=self.Base.get()
        if base!='Decimal':return
        txt=self.Temperature.get()
        if self.temp_btn.cget('text')=='Temp.':        
            self.select_temps(self.Temperature.get())
        else:
            if not self.Disp_List:return
            try:
                if self.Answer_Present.get():
                ...
                if txt=='°F → °C':
                    ...
                    self.expr_update(')')
                self.equal_clicked(txt)
                self.temp_btn.configure(text="Temp.")
            except Exception as e:
                if Language=="English":
    ...
    def select_volume(self, event):
        commands=[]
        for c in range(len(self.Volume_List)):
            commands.append(c)
            commands[c] = lambda arg=self.Volume_List[c]: self.update_volume_btn(arg)
        x = self.winfo_pointerx()
        y = self.winfo_y()
        self.popup_menu = rwPopupMenu(self, caption_list=self.Volume_List ,command_list=commands, font_size=-20, x_pos=x, y_pos=y)
        self.wait_window(self.popup_menu)
        self.popup_menu.grab_release()
        self.popup_menu.destroy()
        if not self.Disp_List:return
        else:self.length_clicked()
    def update_volume_btn(self, txt):
        txt1 = f'{txt} ='
        self.volume_btn.configure(text=txt1)       
        self.Volume.set(txt)
    def volume_clicked(self):
        base=self.Base.get()
        if base!='Decimal':return
        txt=self.Volume.get()
        if self.volume_btn.cget('text') =='Volume' or self.volume_btn.cget('text') =='Volumen':        
            self.select_volume(self.Volume.get())
        else:
            if not self.Disp_List:return
            try:
                if self.Answer_Present.get():
                    ...
                    self.expr_update(str(newval))
                self.reverse_value('expr',True)
                if txt in self.Volume_List:
                    index = self.Volume_List.index(txt)
                    display_txt=txt.split(" → ")[0]
                    self.disp_update(f' {display_txt}')
                    self.expr_update('(')
                    self.return_value('expr',True,True) 
                    self.expr_update(self.Volume_Math[index])
                    self.expr_update(')')
                self.equal_clicked(txt)
                if Language == "English": txt = "Volume"
                else: txt = "Volumen"
                self.volume_btn.configure(text=txt)
            except Exception as e:
                if Language=="English":
                    title="Volume Conversion Error"
                    msg1='Exception occurred while code execution:\n'
                else:    
                    title="Error de Conversión de Volumen"
                    msg1='Ocurrió una excepción durante la ejecución del código:\n'
                msg2= repr(e)
                msg=msg1+msg2
                rwDialog(parent=app, style="msgbox", title=title, prompt=msg, icon="cancel.png")
    ...
    def select_area(self, event):
        commands=[]
        for c in range(len(self.Area_List)):
            commands.append(c)
            commands[c] = lambda arg=self.Area_List[c]: self.update_area_btn(arg)
        x = self.winfo_pointerx()
        y = self.winfo_y()
        self.popup_menu = rwPopupMenu(self, caption_list=self.Area_List ,command_list=commands, font_size=-20, x_pos=x, y_pos=y)
        self.wait_window(self.popup_menu)
        self.popup_menu.grab_release()
        self.popup_menu.destroy()
        if not self.Disp_List:return
        else:self._set_appearance_mode_clicked()
    def update_area_btn(self, txt):
        txt1 = f'{txt} ='
        self.Area.set(txt)
        self.area_btn.configure(text=txt1)
        self.area_btn.update()       
    def area_clicked(self):
        base=self.Base.get()
        if base!='Decimal':return
        txt=self.Area.get()
        if self.area_btn.cget('text') =='Area' or self.area_btn.cget('text') =='Área':        
            self.select_area(self.Area.get())
        else:
            if not self.Disp_List:return
            try:
                if self.Answer_Present.get():
                    ...
                    self.expr_update(str(newval))
                self.reverse_value('expr',True)
                if txt in self.Area_List:
                    index = self.Area_List.index(txt)
                    display_txt=txt.split(" → ")[0]
                    self.disp_update(f' {display_txt}')
                    self.expr_update('(')
                    self.return_value('expr',True,True) 
                    self.expr_update(self.Area_Math[index])
                    self.expr_update(')')
                self.equal_clicked(txt)
                if Language == "English": txt = "Area"
                else: txt == "Área"
                self.area_btn.configure(text=txt)
            except Exception as e:
                if Language=="English":
                    title="Area Conversion Error"
                    msg1='Exception occurred while code execution:\n'
                else:    
                    title="Error de Conversión de Área"
                    msg1='Ocurrió una excepción durante la ejecución del código:\n'
                msg2= repr(e)
                msg=msg1+msg2
                rwDialog(parent=app, style="msgbox", title=title, prompt=msg, icon="cancel.png")
    ...
    def equal_clicked(self, event):
        ...
        if event in self.Length_List or event in self.Temp_List or event in self.Volume_List or event in self.Area_List:
            txt=event.split(" →")[1]
        else:txt=""
        sub_dict={}
    ...
    def button_popups(self, event, txt):# display the popup menu
        ...
        if text != '':rwTimedPopup(self.Display_Text, text=text, text_color="#000000", 
    ...
    def copy_to_clipboard(self, parent, which) -> str:
        ...
        try:
            if which=="selected":
                ...
                if Language=="English":txt=" Selected Text Copied To Clipboard! "
                else:txt="¡Texto Seleccionado Copiado al Portapapeles!"
            elif which=="all":
                text=parent.get("1.0", 'end')
                if text=="\n":text=""
                if Language=="English":txt=" Display Text Copied To Clipboard! "
    ...
    def restart_program(self, lang_change=False):# app
        ...
        try:
            ...
            pass
            os.execl(sys.executable, os.path.abspath("ctkCalculator.exe"), *sys.argv)
    def show_ratio_calculus_menu(self):
        if Language=="English":
            captions = ['Ratio / Porportion Calculator', 'Integral Calculator', 'Derivative Calculator']
        else: 
            captions = ['Calculadora de Ratio / Proporciones', 'Calculadora de Integrales', 'Calculadora de Derivadas']
        commands = [lambda:rwRatioCalculator(self), lambda i="integrate": self.do_calculus(i), lambda i="differentiate": self.do_calculus(i)]        
        y = self.winfo_y()
        self.popup_menu = rwPopupMenu(self, captions ,commands, -20, self.winfo_pointerx(), y)
    ...
    def show_menu_popup(self, event):# app
        ...
        self.popup_menu = rwPopupMenu(self, captions ,commands, -20, self.winfo_pointerx(), self.winfo_y())
        self.popup_menu.bind("<Button-3>", lambda e: self.popup_menu.withdraw())
        self.wait_window(self.popup_menu)
        try:
            self.popup_menu.grab_release()
    ...
    def write_setup(self):
        ...
        temp_dict[0]=Language
        temp_dict[1]=Default_DPI
        temp_dict[2]=str(self.Theme.get())
        temp_dict[3]=self.geometry()
        family_now=self.Display_Font.cget("family")
        size_now=self.Display_Font.cget("size")
        weight_now=self.Display_Font.cget("weight")
        slant_now=self.Display_Font.cget("slant")
        temp_dict[4]=str({'family': family_now, 'size': size_now, 'weight': weight_now, 'slant': slant_now})
        temp_dict[5]=str(self.Display_fg.get())
        temp_dict[6]=str(self.Display_bg.get())
        temp_dict[7]=str(mp.dps)
        temp_dict[8]=str(self.Round.get())
        temp_dict[9]=str(self.Exp_Precision.get())
        temp_dict[10]=str(self.Exp_Digits.get())
        temp_dict[11]=str(self.Bit_Size.get())
    ...
    def read_setup(self):
        ...
        try:
            ...
            for key, value in data.items():
                global Language
                if key=="0":Language = value
                elif key=="1":
                    global Default_DPI 
                    Default_DPI = value
                elif key=="2":self.Theme.set(value)
                    ...
                    self.Display_Text.focus_force()
                    self.Display_Text.configure(fg_color=self.Display_bg.get())
                elif key=="7":mp.dps=int(value)
                elif key=="8":self.Round.set(int(value))
                elif key=="9":self.Exp_Precision.set(int(value))
                elif key=="10":self.Exp_Digits.set(int(value))
                elif key=="11":self.Bit_Size.set(int(value))
        except Exception as e:
            if Language=="English":
    ...
    def clear_all(self):
        ...
        if Language == 'English': 
            self.length_btn.configure(text="Length")
            self.temp_btn.configure(text="Temp.")
            self.volume_btn.configure(text="Volume")
            self.area_btn.configure(text="Area")
            self.select_btn.configure(text="Select")
        else:
            self.length_btn.configure(text="Longitud")
            self.temp_btn.configure(text="Temp.")
            self.volume_btn.configure(text="Volumen")
            self.area_btn.configure(text="Área")
            self.select_btn.configure(text="Selec.")
    def set_defaults(self):
        temp_dict=json.load(open("Config.json", "r+"))
        if len(temp_dict)==2:# Only Language and DPI Exist
            mp.dps=50
            self.Round.set(15)
        ...
        if Language=="English":
            ...
            if status == "open":
                ...
                if current_datetime.hour<12:
                    ...
                    greeting=f"Adiós {self.user}. ¡Que Tengas una Maravillosa Tarde!"
                elif current_datetime.hour>=17 and current_datetime.hour<=24:
                ...
                if current_datetime.hour<12:
                    ...
                    greeting=f"Buenas Noches {self.user}."
        rwTimedPopup(self.Display_Text, text=greeting, text_color="#000000", 
                            fg_color="#14fafa", font_size=-24, delay_time=1500)
