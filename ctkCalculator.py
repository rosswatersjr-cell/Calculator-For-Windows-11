...
class MyFontChooser(ctk.CTkToplevel):
    def __init__(self, parent, font_dict={}, text="Sample Text For Selected Font", **kwargs):
        ...
        if os.path.exists(fontfile):# Windows Font Family Names are stored in json file. Not reliant on tk families.
            ...
            if Language == "English":
                title=f"{parent.root_title}  Font Chooser"
                self.sample_txt = text
            else:
                title=title=f"{parent.root_title}  Selector de Fuentes"
            ...
            font_size = int(-16 * self.factor) 
            self.chooser_font = ctk.CTkFont(family="Arial", size=font_size, weight="normal", slant="roman")
            ...
            msg=msg1+msg2
            MyDialog(parent=app, style="msgbox", title=title, prompt=msg, icon="cancel.png")
            self.quit()
...
class Ratio_Calculator(ctk.CTkToplevel):
    def __init__(self, app):
        ...
        try:
            ...
            self.factor = (Default_DPI / 96.0)  # 96 is standard DPI for 1 point
            font_size = int(-16 * self.factor) 
            self.ratio_font=ctk.CTkFont(family='Arial', size=font_size, weight='normal', slant='italic')# Ratio Formulas
            self.formula=""
    ...
    def calculate_ratio(self,a_1,a_2,b_1,b_2,a_3,b_3):
        try:
            ...
            if count < 3:
                ...
                msg=msg1+msg2+msg3
                MyDialog(parent=app, style="msgbox", title=title, prompt=msg, icon="info.png")
...
class Calculus(ctk.CTkToplevel):
    def __init__(self, app, funct):
        ...
        try:
            ...
            self.factor = (Default_DPI / 96.0)  # 96 is standard DPI for 1 point
            font_size = int(-18 * self.factor) 
            calculus_font = ctk.CTkFont(family="Arial", size=font_size)
            ...
            self.symbol_bx = ctk.CTkComboBox(calculus.window, values=self.used_symbols, button_color=("#00cccc","#00cccc"), 
                                            ...
                                            dropdown_text_color=("black", "black"), font=calculus_font, dropdown_font=calculus_font) 
...
class MyDialog(ctk.CTkToplevel):
    def __init__(self, parent, style: Literal["msgbox", "entry"], title, prompt, choices=None, 
        ...
        if self.style == "entry": 
            if self.choices is not None:
                if type(self.choices)==list:# List
                    ...
                    self.combobox = ctk.CTkComboBox(self, values=self.choices, font=self.mydialog_font, command=self.on_select) 
        ...
        if Language == "English": 
            ...
            txt1 = "Vale"
            txt2 = "Cancelar"
        self.ok_button = ctk.CTkButton(self.button_frame, text=txt1, font=self.mydialog_font, command=self.on_ok)
        self.ok_button.pack(side="left", padx=10)
        self.cancel_button = ctk.CTkButton(self.button_frame, text=txt2, font=self.mydialog_font, command=self.on_cancel)
...
class Calculator(ctk.CTk):
    def __init__(self):
        ...
        for f in range(len(std_sizes)):
            font_sizes.append(int(std_sizes[f] * factor))  # adjust for high‑DPI        
        ...
        if len(temp_dict)==1 or temp_dict["0"] == "" :# Only Theme Exist Or No Language
            ...
            msg=msg1+msg2
            while Language == "": 
                lang = MyDialog(parent=self, style="entry", title=title, prompt=msg, choices=languages, icon="setup.png")
        ...
        self.us_metric_btn = ctk.CTkButton(self.pad_fra, text=self.US_Metric.get(), anchor="c", border_width=2, corner_radius=5, font=self.temp_font,
                    fg_color=("#80ff80", "#80ff80"), hover_color="#00ced1", text_color="black",
        ...
        self.temp_btn = ctk.CTkButton(self.pad_fra, text=self.Selected_Temp.get(), anchor="c", border_width=2, corner_radius=5, font=self.temp_font,
                    fg_color=("#ff9c95", "#ff9c95"), hover_color="#00ced1", text_color="black",
    ...
    def precision(self, which): # Calculator Precision Configuration
        ...
        if which=='dp':
            dpp=MyDialog(self, title=titles[0], style="entry", prompt=messages[0], choices=None, init_val=mp.dps, min_val=1, max_val=500, icon="setup.png")
            ...
            rnd=MyDialog(self, title=titles[1], style="entry", prompt=messages[1], choices=None, init_val=self.Round.get(), min_val=0, max_val=500, icon="setup.png")
            if rnd.result is not None:
                while int(rnd.result) < 0 or int(rnd.result) > 500: 
                    rnd=MyDialog(self, title=titles[1], style="entry", prompt=messages[1], choices=None, init_val=self.Round.get(), min_val=0, max_val=500, icon="setup.png")
                self.Round.set(int(rnd.result))
        elif which=='exp':        
            snp=MyDialog(self, title=titles[2], style="entry", prompt=messages[2], choices=None, init_val=self.Exp_Precision.get(), min_val=1, max_val=500, icon="setup.png")
            ...
            enp=MyDialog(self, title=titles[3], style="entry", prompt=messages[3], choices=None, init_val=self.Exp_Digits.get(), min_val=1, max_val=4, icon="setup.png")
            if enp.result is not None:
                while int(enp.result) < 1 or int(enp.result) > 4: 
                    enp=MyDialog(self, title=titles[3], style="entry", prompt=messages[3], choices=None, init_val=self.Exp_Digits.get(), min_val=1, max_val=4, icon="setup.png")
                self.Exp_Digits.set(int(enp.result))
        elif which=='bit_size':
            bbs=MyDialog(self, title=titles[4], style="entry", prompt=messages[4], choices=['8','16','32','64','128','256','512'], init_val=self.Bit_Size.get(), min_val=None, max_val=None, icon="setup.png")
    ...
    def choose_font(self): # Calculator Display Font
        try:
            self.popup_menu.withdraw()
            app.update()
            if Language=="English":txt=["Sample text for selected font","Choose Display Font"]
    ...
    def choose_theme(self, theme):
        ...
        theme=MyDialog(self, title=" < Set Color Theme >", style="entry", prompt=msg, choices=["System", "Dark", "Light"], init_val=self.Theme.get(), icon="setup.png")
    ...
    def choose_color(self, which): # Display fg and bg Colors
        ...
        _x = int(((screen_width / 2) - ((_width * scale_factor) / 2)))
        _y = int(((screen_height / 2) - ((_height * scale_factor) / 2)))
        if which=='fg_color':
        ...
        dialog.geometry(f"{_width}x{_height}+{_x}+{_y}")
    ...
    def select_distance(self, event):
        ...
        y = int(self.winfo_pointery() - self.height)
    ...
    def button_popups(self, event, txt):# display the popup menu
        ...
        MyTimedPopup(self.Display_Text, text=text, text_color="#000000", 
        ...
        if Language=="English":
            ...
            if status == "open":
                ...
                if current_datetime.hour<12:
                    ...
                    greeting=f"Buenas Noches {self.user}."
        MyTimedPopup(self.Display_Text, text=greeting, text_color="#000000", 
