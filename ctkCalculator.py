...
class rwColorPicker(ctk.CTkToplevel):
    def __init__(self, master=None, init_color="#00aaff", title="Choose Color"):
        ...
        self.hex_font = ctk.CTkFont(family="Arial", size=self.hex_font_size, weight="bold", slant="roman")
        self.hue = 0.0
        self.sat = 1
        self.val = 1
        self.result = None
        ...
        self.marker = None
        self.scale = 1
        self.marker_radius = 7 * self.scale
        self.canvas_width = 200.0
        self.canvas_height = 200.0
        ...
        self.canvas_picker.pack(expand=True, pady=10)
        ...
        rgb_frame.pack(expand=True, pady=10, fill="x")
        self.hue_slider = ctk.CTkSlider(rgb_frame, from_=0, to=1, height=25,  fg_color='yellow',
                                        button_hover_color='aqua',  progress_color='purple', command=self.hue_change)
        self.r_slider = ctk.CTkSlider(rgb_frame, from_=0, to=255, height=25, fg_color='gray', 
                                      button_hover_color='aqua', progress_color='red', command=self.red_change)
        self.g_slider = ctk.CTkSlider(rgb_frame, from_=0, to=255, height=25, fg_color='gray', 
                                      button_hover_color='aqua', progress_color='green', command=self.green_change)
        self.b_slider = ctk.CTkSlider(rgb_frame, from_=0, to=255, height=25, fg_color='gray', 
                                      button_hover_color='aqua', progress_color='blue', command=self.blue_change)
        self.hue_lbl = ctk.CTkLabel(rgb_frame, text='Hue',  font=self.chooser_font)
        self.hue_lbl.pack(expand=True, pady=5)
        ...
        self.hex_label = ctk.CTkLabel(self, text="#000000", font=self.hex_font)
        self.hex_label.pack(expand=True, pady=5)
        ...
        self.ok_btn.pack(expand=True, side="left", padx=(10,30))
        ...
        self.cancel_btn.pack(expand=True, side="left", padx=(30,10))
    ...
    def picker_clicked(self, event):
        ...
        self.marker = self.canvas_picker.create_oval(x - r, y - r, x + r, y + r, outline="white", width=2)
    ...
    def update_preview(self):
        ...
        self.hex_label.configure(text=self.hex_color)
        self.r = int(r * 255)
        self.g = int(g * 255)
        self.b = int(b * 255)
    ...
    def handle_resize(self):
        try:    
            ...
            return
    def on_ok(self):
        self.result = self.hex_label.cget("text")
        self.destroy()
    def on_cancel(self):
        self.result = None
        self.destroy()
    ...
    def __init__(self, parent, font_dict={}, text="Sample Text For Selected Font", **kwargs):
        ...
        if os.path.exists(fontfile):# Windows Font Family Names are stored in json file. Not reliant on tk families.
            ...
            try:
                ...
                fake_txt=["X" * max_length, "XXXX", "X * 22"]# Font Family ListBox Width And Font Weight ListBox
                req_width=[]
                for w in range(len(fake_txt)):
                    ...
                    self.update()
                    req_width.append(w)
                    req_width[w]=fake_lbl.winfo_reqwidth()
                    req_height=(fake_lbl.winfo_reqheight() * 7)# Show 6 Selections
                fake_lbl.destroy()
                self.entry_family = ctk.CTkEntry(self, placeholder_text="Enter Name", font=self.chooser_font, textvariable=self.var_family,
                                            width=req_width[0], corner_radius=10, state="normal")
                self.entry_size = ctk.CTkEntry(self, placeholder_text="Enter Size", font=self.chooser_font, textvariable=self.var_size, 
                                            width=req_width[1], corner_radius=10, state="normal")
                if Language == "English": 
                    txt = "Font Options"
                else: 
                    txt = "Opciones de Fuente"
                self.font_options = ctk.CTkEntry(self, placeholder_text=txt, font=self.chooser_font,  
                                            width=req_width[2], corner_radius=10, state="normal")
                self.font_options.insert(0, txt)
                self.entry_family.grid(row=0, column=0, sticky="ew", columnspan=4, rowspan=1, pady=(10, 2), padx=(10, 0))
                self.entry_size.grid(row=0, column=4, sticky="ew", columnspan=2, rowspan=1, pady=(10, 2), padx=(10, 0))
                self.font_options.grid(row=0, column=6, sticky="ew", columnspan=5, padx=(10, 10), pady=(10, 2))
                self.list_family = CTkListbox(self, font=self.chooser_font, width=req_width[0], height=req_height,
                                                border_width=2, justify="left", corner_radius=10)
                self.sizes = ["%i" % i for i in (list(range(6, 41)) + list(range(42, 74, 2)))]
                self.list_size = CTkListbox(self, font=self.chooser_font, width=req_width[1], height=req_height, 
                                    border_width=2, justify="left", corner_radius=10)
                self.list_family.grid(row=1, column=0, sticky="nsew", columnspan=4, rowspan=1, pady=(10, 2), padx=(10, 0))
                self.list_size.grid(row=1, column=4, sticky="nsew", columnspan=2, rowspan=1, pady=(10, 2), padx=(10, 0))
                self.options_frame=ctk.CTkFrame(self,  border_width=2, corner_radius=10, width=req_width[2], height=req_height) 
                self.options_frame.grid(row=1, column=6, sticky="nsew", columnspan=5, rowspan=1, pady=(10, 2), padx=(10, 10))
                ...
                b_bold.grid(row=0, sticky="w", padx=4, pady=(5, 5))
                ...
                b_italic.grid(row=1, sticky="w", padx=4, pady=(5, 5))
                ...
                b_underline.grid(row=2, sticky="w", padx=4, pady=(5, 5))
                ...
                b_overstrike.grid(row=3, sticky="w", padx=4, pady=(5, 5))
                ...
                self.preview.grid(row=2, column=0, columnspan=11, rowspan=1, sticky="nsew", padx=(10, 10), pady=(10, 10))
                button_frame=ctk.CTkFrame(self) 
                button_frame.grid(row=3, column=0, columnspan=11, padx=(10, 10), pady=(10, 10))
                if Language == "English": 
                    txt1 = "OK"
                    txt2 = "Cancel" 
                else: 
                    txt1 = "Vale"
                    txt2 = "Cancelar"
                self.ok_btn = ctk.CTkButton(button_frame, text=txt1, border_width=2, corner_radius=5, font=self.chooser_font,
                                border_color="black",  hover_color="#00ced1", 
                                command=self.ok).grid(row=0, column=0, padx=(0, 20), sticky='nsew')  
                self.cancel_btn = ctk.CTkButton(button_frame, text=txt2, border_width=2, corner_radius=5, font=self.chooser_font,
                                border_color="black", hover_color="#00ced1", 
                                command=self.quit).grid(row=0, column=1, padx=(20, 0), sticky='nsew') 
                ...
                self.update_idletasks()
                self.req_height = self.winfo_reqheight()
                self.req_width = self.winfo_reqwidth() + 100
                self.x = int((self.screen_width / 2) - (self.req_width / 2))
                self.y = int((self.screen_height / 2) - (self.req_height / 2))
                self.geometry(f"{self.req_width}x{self.req_height}+{self.x}+{self.y}")
...
class Calculator(ctk.CTk):
    def __init__(self):
        ...
        self.Display_Font=ctk.CTkFont(family='Arial', size=self.base_font_sizes[4], weight='normal', slant='italic')# Display Only
        temp_dict=json.load(open("Config.json", "r+"))
        global Language
        Language = ""
    ...
    def handle_resize(self):
        try:    
            ...
            self.Display_Font.configure(size=new_sizes[4])# Display Only
        except Exception:
            return
