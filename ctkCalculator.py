...
class MyFontChooser(ctk.CTkToplevel):
    def __init__(self, parent, font_dict={}, text="Sample text for selected font", **kwargs):
        ...
        if os.path.exists(fontfile):# Windows Font Family Names are stored in json file. Not reliant on tk families.
            ...
            self.y = int(((self.screen_height / 2) - ((self.height * self.scale_factor) / 2)) - 2 * self.min_window)
            self.geometry(f"{self.width}x{self.height}+{self.x}+{self.y}")
            self.chooser_font = ctk.CTkFont(family="Arial", size=-16, weight="normal", slant="roman")
            self.update()
            ...
            try:
                ...
                for w in range(len(fake_txt)):
                    ...
                    req_height=(fake_lbl.winfo_reqheight() * 7)# Show 7 Selections
                fake_lbl.destroy()
                ...
                options_frame=ctk.CTkFrame(self,  border_width=2, corner_radius=10, height=200) 
                self.font_size = ctk.StringVar(self, " ".join(self.sizes))
                self.var_bold = ctk.BooleanVar(self, self.font_dict["weight"] == "bold")
                b_bold = ctk.CTkCheckBox(options_frame, text=[font_props[0]], font=self.chooser_font, border_width=2,
                ...
                b_italic = ctk.CTkCheckBox(options_frame, text=[font_props[1]], font=self.chooser_font, border_width=2,
                ...
                b_underline = ctk.CTkCheckBox(options_frame, text=[font_props[2]], font=self.chooser_font, border_width=2,
                ...
                b_overstrike = ctk.CTkCheckBox(options_frame, text=[font_props[3]], font=self.chooser_font, border_width=2,
                ...
                iconfile=os.path.join(self.base_path, "setup.png")
                pil_icon = Image.open(iconfile)
                ctk_image = ctk.CTkImage(light_image=pil_icon, dark_image=pil_icon, size=(icon_size, icon_size)) 
                icon_label = ctk.CTkLabel(options_frame, image=ctk_image, text="")
                icon_label.grid(row=4, sticky="w", padx=(10, 0), pady=(5, 5))
                ...
                self.list_size = CTkListbox(self, height= req_height, width=req_width[1], font=self.chooser_font, 
                                    border_width=2, justify="left", corner_radius=10)
                ...
                options_frame.grid(rowspan=3, row=1, column=2, sticky="nsew", pady=(0, pad_y2), padx=(10, 0))
...
class Ratio_Calculator(ctk.CTkToplevel):
    def __init__(self, app):
        ...
        try:
            ...
            self.ratio_font=ctk.CTkFont(family='Arial', size=-16, weight='normal', slant='italic')# Ratio Formulas
            self.formula=""
    ...
    def copy_display(self):
        if hasattr(self, "popup_btn") and self.popup_btn.winfo_exists():
            ...
            return
        scale_factor = ctypes.windll.shcore.GetScaleFactorForDevice(0) / 100
        parent_wid=self.formula_text.winfo_width() / scale_factor
        parent_hgt=self.formula_text.winfo_height() / scale_factor
...
class Calculus(ctk.CTkToplevel):
    def __init__(self, app, funct):
        ...
        try:
            ...
            self.min_window = ctypes.windll.user32.GetSystemMetrics(58)
            self.width = int(self.screen_width * 0.4)
            self.height = int(self.screen_height * 0.46)
            self.scale_factor = self._get_window_scaling()
            self.x = int(((self.screen_width / 2) - ((self.width * self.scale_factor) / 2)))
            self.y = int(((self.screen_height / 2) - ((self.height * self.scale_factor) / 2)) - 2 * self.min_window)
            self.geometry(f"{self.width}x{self.height}+{self.x}+{self.y}")
...
class MyDialog(ctk.CTkToplevel):
    def __init__(self, parent, 
        ...
        self.scale_factor = self._get_window_scaling()
        self.x = int(((self.screen_width / 2) - ((self.width * self.scale_factor) / 2)))
        self.y = int(((self.screen_height / 2) - ((self.height * self.scale_factor) / 2))) - self.min_window
        self.geometry(f"{self.width}x{self.height}+{self.x}+{self.y}")
        self.attributes("-topmost", True)
        self.update()
        self.mydialog_font = ctk.CTkFont(family="Arial", size=-16, weight="normal", slant="roman")
        self.result = None
...
class Calculator(ctk.CTk):
    def __init__(self, *args, **kwargs):
        ...
        self.ce=ctk.CTkButton(self.pad_fra, text='CE', anchor="c", border_width=2, corner_radius=5, font=self.symbols_font, 
                        hover_color="#00ced1", text_color=("black","white"),) # Clear Entry Button 
    ...
    def select_distance(self, event):
        ...
        y = int(self.winfo_pointery() - (self.height * scale_factor))
    ...
    def copy_to_clipboard(self, parent, which) -> str:
        ...
        try:
            ...
            if text!="":
                ...
                scale_factor = ctypes.windll.shcore.GetScaleFactorForDevice(0) / 100
                parent_wid=parent.winfo_width() / scale_factor
                parent_hgt=parent.winfo_height() / scale_factor
