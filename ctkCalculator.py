...
class Calculator(ctk.CTk):
    def __init__(self, *args, **kwargs):
        ...
        self.us_metric_btn = ctk.CTkButton(self.pad_fra, text=self.US_Metric.get(), anchor="c", border_width=2, corner_radius=5, font=self.temp_font,
                    ...
                    textvariable=self.US_Metric, command=lambda: self.us_metric_clicked())  
        self.us_metric_btn.place(relx=0.137, rely=0.855, relwidth=0.117, relheight=0.12,)
        self.us_metric_btn.bind('<Button-3>', lambda event, txt=self.US_Metric.get(): self.select_us_metric(txt))
    ...
    def log_clicked(self, text):
        ...
        try:       
            ...
            cancel_dialog=MyDialog(parent=app, style="msgbox", title=title, prompt=msg, icon="cancel")
            return 'break'
    def select_us_metric(self, event):
        conv_us_metric = ['in. → mm', 'mm → in.', 'in. → cm', 'cm → in.','ft → m', 'm → ft', 'mi → km', 'km → mi', 
                          'yds → m', 'm → yds', 'ft → yds', 'yds → ft', 'mi → au', 'km → au', 'au → mi', 'au → km']
        commands=[lambda arg=conv_us_metric[0]: self.update_us_metric_btn(arg), lambda arg=conv_us_metric[1]: self.update_us_metric_btn(arg), 
                  lambda arg=conv_us_metric[2]: self.update_us_metric_btn(arg), lambda arg=conv_us_metric[3]: self.update_us_metric_btn(arg), 
                  lambda arg=conv_us_metric[4]: self.update_us_metric_btn(arg), lambda arg=conv_us_metric[5]: self.update_us_metric_btn(arg),
                  lambda arg=conv_us_metric[6]: self.update_us_metric_btn(arg), lambda arg=conv_us_metric[7]: self.update_us_metric_btn(arg), 
                  lambda arg=conv_us_metric[8]: self.update_us_metric_btn(arg), lambda arg=conv_us_metric[9]: self.update_us_metric_btn(arg), 
                  lambda arg=conv_us_metric[10]: self.update_us_metric_btn(arg), lambda arg=conv_us_metric[11]: self.update_us_metric_btn(arg),
                  lambda arg=conv_us_metric[12]: self.update_us_metric_btn(arg), lambda arg=conv_us_metric[13]: self.update_us_metric_btn(arg),
                  lambda arg=conv_us_metric[14]: self.update_us_metric_btn(arg), lambda arg=conv_us_metric[15]: self.update_us_metric_btn(arg)]
        x = self.winfo_pointerx() - ((70 / self.width) * self.width)
        y = self.winfo_pointery() - self.height
        self.popup_menu = MyPopupMenu(self, caption_list=conv_us_metric ,command_list=commands, font_size=16, x_pos=x, y_pos=y)
        self.wait_window(self.popup_menu)
        self.popup_menu.grab_release()
        self.popup_menu.destroy()
        if not self.Disp_List:return
        else:self.us_metric_clicked()
    def update_us_metric_btn(self, txt):
        self.us_metric_btn.configure(text=txt)       
        self.US_Metric.set(txt)
    def us_metric_clicked(self):
        base=self.Base.get()
        if base!='Decimal':return
        ...
        try:
            ...
            self.reverse_value('expr',True)
            if txt=='in. → mm':# in. / 0.0393700787401575
                self.disp_update(' in.')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' / 0.0393700787401575')
                self.expr_update(')')
            elif txt=='mm → in.':# cm * 0.0393700787401575
                self.disp_update(' mm')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' * 0.0393700787401575')
                self.expr_update(')')
            elif txt=='in. → cm':# in. / 0.393700787401575
                self.disp_update(' in.')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' / 0.393700787401575')
                self.expr_update(')')
            elif txt=='cm → in.':# cm * 0.393700787401575
                self.disp_update(' cm')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' * 0.393700787401575')
                self.expr_update(')')
            elif txt=='ft → m':# ft / 3.28083989501312
                self.disp_update(' ft')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' / 3.28083989501312')
                self.expr_update(')')
            elif txt=='m → ft':#  m * 3.28083989501312
                self.disp_update(' m')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' * 3.28083989501312')
                self.expr_update(')')
            elif txt=='mi → km':# mi / 0.621371192237334
                self.disp_update(' mi')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' / 0.621371192237334')
                self.expr_update(')')
            elif txt=='km → mi':# km * 0.621371192237334 
                self.disp_update(' km')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' * 0.621371192237334')
                self.expr_update(')')
            elif txt=='yds → m':# yds / 1.09361329833771
                self.disp_update(' yds')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' / 1.09361329833771')
                self.expr_update(')')
            elif txt=='m → yds':# m * 1.09361329833771
                self.disp_update(' m')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' * 1.09361329833771')
                self.expr_update(')')
            elif txt=='ft → yds':# ft / 3
                self.disp_update(' ft')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' / 3.0')
                self.expr_update(')')
            elif txt=='yds → ft':# yds * 3
                self.disp_update(' yds')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' * 3.0')
                self.expr_update(')')
            elif txt=='mi → au':# mi * 1.0757800178347E-8
                self.disp_update(' mi')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' * 1.0757800178347e-8')
                self.expr_update(')')
            elif txt=='km → au':# mi * 6.684587122E-9
                self.disp_update(' km')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' * 6.68458712226845e-9')
                self.expr_update(')')
            elif txt=='au → mi':# au / 1.0757800178347E-8
                self.disp_update(' au')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' / 1.0757800178347e-8')
                self.expr_update(')')
            elif txt=='au → km':# au / 6.68458712226845e-9
                self.disp_update(' au')
                self.expr_update('(')
                self.return_value('expr',True,True) 
                self.expr_update(' / 6.68458712226845e-9')
    ...
    def select_temps(self, event):
        conv_degrees = ['°F → °C', '°F → °K','°F → °R', '°C → °F', '°C → °K', '°C → °R', '°K → °F','°K → °C','°K → °R', '°R → °F','°R → °C','°R → °K']
        commands=[lambda arg=conv_degrees[0]: self.update_temp_btn(arg), lambda arg=conv_degrees[1]: self.update_temp_btn(arg), lambda arg=conv_degrees[2]: self.update_temp_btn(arg),
                  lambda arg=conv_degrees[3]: self.update_temp_btn(arg), lambda arg=conv_degrees[4]: self.update_temp_btn(arg), lambda arg=conv_degrees[5]: self.update_temp_btn(arg),
                  lambda arg=conv_degrees[6]: self.update_temp_btn(arg), lambda arg=conv_degrees[7]: self.update_temp_btn(arg), lambda arg=conv_degrees[8]: self.update_temp_btn(arg),
                  lambda arg=conv_degrees[9]: self.update_temp_btn(arg), lambda arg=conv_degrees[10]: self.update_temp_btn(arg), lambda arg=conv_degrees[11]: self.update_temp_btn(arg)]
        x = self.winfo_pointerx() - ((70 / self.width) * self.width)
        y = self.winfo_pointery() - self.height
        self.popup_menu = MyPopupMenu(self, caption_list=conv_degrees ,command_list=commands, font_size=16, x_pos=x, y_pos=y)
        self.wait_window(self.popup_menu)
        self.popup_menu.grab_release()
        self.popup_menu.destroy()
        if not self.Disp_List:return
    ...
    def equal_clicked(self, event):
        ...
        if event=='°F → °C':txt='°C'# Add Units To Answer If Converions
        elif event=='°F → °K':txt='°K'
        elif event=='°F → °R':txt='°R'
        elif event=='°C → °F':txt='°F'
        elif event=='°C → °K':txt='°K'
        elif event=='°C → °R':txt='°R'
        elif event=='°K → °F':txt='°F'
        elif event=='°K → °C':txt='°C'
        elif event=='°K → °R':txt='°R'
        elif event=='°R → °F':txt='°F'
        elif event=='°R → °C':txt='°C'
        elif event=='°R → °K':txt='°K'
        elif event=='in. → mm':txt=' mm'
        elif event=='mm → in.':txt=' in.'
        elif event=='in. → cm':txt=' cm'
        elif event=='cm → in.':txt=' in.'
        elif event=='ft → m':txt=' m'
        elif event=='m → ft':txt=' ft'
        elif event=='mi → km':txt=' km'
        elif event=='km → mi':txt=' mi'
        elif event=='yds → m':txt=' m'
        elif event=='m → yds':txt=' yds'
        elif event=='ft → yds':txt=' yds'
        elif event=='yds → ft':txt=' ft'
        elif event=='mi → au':txt=' au'
        elif event=='km → au':txt=' au'
        elif event=='au → mi':txt=' mi'
        elif event=='au → km':txt=' km'
        else:txt=""
        sub_dict={}
