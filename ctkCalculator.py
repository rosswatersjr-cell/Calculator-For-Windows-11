...
class MyFontChooser(ctk.CTkToplevel):
    def __init__(self, parent, font_dict={}, text="Abcd", **kwargs):
        ...
        self.title(title)
        self.geometry('%dx%d+%d+%d' % (parent.width, parent.height, parent.x, parent.y))
        self.update()
        fontchooser_font = ctk.CTkFont(family="Arial", size=14, weight="normal", slant="roman")
        ...
        try:
            ...
            for w in range(len(fake_txt)):
                fake_lbl= ctk.CTkLabel(master=self, text=fake_txt[w], font=fontchooser_font)# Just To Get Required Width And Height
                self.update()
                req_width.append(w)
                req_width[w]=fake_lbl.winfo_reqwidth() / parent.scale_factor
                req_height=(fake_lbl.winfo_reqheight() * 7)  / parent.scale_factor# Show 7 Selections
            fake_lbl.destroy()
            ...
            self.list_family = CTkListbox(self, height= req_height, width=req_width[0], font=fontchooser_font, fg_color="#99ffff", text_color="#000000",
                                border_width=2, justify="left", hover_color="#ff8080", corner_radius=10)
            self.list_size = CTkListbox(self, height= req_height, width=req_width[1], font=fontchooser_font, fg_color="#99ffff", text_color="#000000",
                                border_width=2, justify="left", hover_color="#ff8080", corner_radius=10)
            ...
            self.list_family.select(i)
            self.list_family.see(i)
            try:
                i = self.sizes.index(self.entry_size.get())
                self.list_size.selection_clear()
                self.list_size.select(i)
                self.list_size.see(i)
            except ValueError:# size not in list
                pass
            self.entry_family.grid(row=0, column=0, sticky="ew", pady=(10, 2), padx=(10, 0))
            self.entry_size.grid(row=0, column=2, sticky="ew", pady=(10, 2), padx=(10, 0))
            self.list_family.grid(row=1, column=0, sticky="nsew", pady=(0, 0), padx=(10, 0))
            self.list_size.grid(row=1, column=2, sticky="nsew", pady=(0, 0), padx=(10, 0))
            options_frame.grid(row=0, column=4, rowspan=2, padx=10, pady=10, ipadx=10)
            self.preview.grid(row=2, column=0, columnspan=5, sticky="eswn", padx=10, pady=(10, 10), ipadx=4, ipady=4)
    ...
    def quit(self):
        ...
        self.destroy()    
def do_ratios(txt):
    if txt == "A : B": ratio=Ratio_Calculator(app)
class Ratio_Calculator(ctk.CTkToplevel):
    def __init__(self, app):
        ...
        try:
            ...
            formula_text.bind("<Control-c>", lambda event:app.copy_to_clipboard(self,formula_text,"selected",event))    
            formula_text.bind("<Button-3>", self.show_ratio_popup)
            formula_text.focus()
            ...
            self.destroy    
    def show_ratio_popup(self, event):
            if app.Language.get()=="English":captions=['Copy Displayed Text To Clipboard']
            else:captions=['Copiar Texto Mostrado al Portapapeles']
            commands=[lambda arg1=self, arg2=formula_text, arg3="all": app.copy_to_clipboard(arg1, arg2, arg3)]
            x = app.winfo_pointerx()
            y = app.winfo_pointery()
            self.popup_menu = CustomPopupMenu(formula_text, captions ,commands, 16, x, y)
    ...
    def calculate_ratio(self,a_1,a_2,b_1,b_2,a_3,b_3):
        try:
            ...
            if count < 3:
                ...
                msg=msg1+msg2+msg3
                info_dialog=MyDialog(parent=app, dialog_type="msgbox", title=title, prompt=msg, icon="info")
            ...
            msg2=repr(e)
            msg=msg1+msg2
            cancel_dialog=MyDialog(parent=app, dialog_type="msgbox", title=title, prompt=msg, icon="cancel")    
...
class MyDialog(ctk.CTkToplevel):
    def __init__(self, parent, 
                 dialog_type: Literal["msgbox", "entry"], 
                 title, prompt, 
                 choices=None, 
        ...
        self.width = int(parent.screen_width*0.25)
        self.height = int(parent.screen_height*0.25)
        x = int(((parent.screen_width // 2) - (self.width // 2)) / parent.scale_factor)
        y = int(((parent.screen_height // 2) - (self.height // 2)) / parent.scale_factor)
    ...
    def on_cancel(self):
        ...
        self.destroy() # Close the dialog
        return self.result    
class TimedPopup(ctk.CTkLabel):
    # If x_pos And y_pos doesn's exist, The Popup Is Centered On The Parent Widget. 
    def __init__(self, parent, 
                 text, text_color, 
                 fg_color, font_size, 
                 delay_time,
        ...
        try:
            text=(f" {text} ")
            popup_font = ctk.CTkFont(family="Arial", size=font_size, weight="normal", slant="roman")
            fake_lbl= ctk.CTkLabel(master=parent, text=text, font=popup_font)# Just To Get Required Width And Height
            parent.update()
            req_width=fake_lbl.winfo_reqwidth()
            req_height = fake_lbl.winfo_reqheight()
            self.scale_factor = ctypes.windll.shcore.GetScaleFactorForDevice(0) / 100
            if x_pos == None:
                parent_wid=parent.winfo_width() / self.scale_factor
                parent_center_x=(parent_wid / 2)
                x_pos=abs((parent_center_x - (req_width / 2)))
            if y_pos == None:
                add_y = int(abs(0.0 - (16.0 * (1.0 - self.scale_factor))))# Y Pos Not Centering Correctly, 1=0,1.5=8,1.75=12
                parent_hgt=parent.winfo_height() / self.scale_factor
                parent_center_y=parent_hgt / 2
                y_pos=abs((parent_center_y-(req_height / 2))) + add_y
            fake_lbl.destroy()    
            super().__init__(master=parent, text=text, width=req_width, text_color=text_color, 
                            fg_color=fg_color, corner_radius=10, font=popup_font)
            self.place(x=x_pos, y=y_pos)
            self.update()
            self.after(delay_time, self.destroy)
        except Exception as e:
            self.destroy    
class CustomPopupMenu(ctk.CTkToplevel):
    def __init__(self, parent, caption_list, command_list, font_size, x_pos, y_pos):
        ...
        try:
            max_wid=0
            for i in range(len(caption_list)):# Get Max Text Length For Menu Width
                txt = f"XXXX{caption_list[i]}XXX"
                if len(txt)>max_wid:
                    max_wid = len(txt)
                    text = txt
            self.menu_font = ctk.CTkFont(family="Arial", size=font_size, weight="normal", slant="roman")
            fake_btn= ctk.CTkLabel(self, text=text, font=self.menu_font)# Just To Get Required Width And Height
            parent.update()
            req_width=fake_btn.winfo_reqwidth()
            req_height=fake_btn.winfo_reqheight() * len(caption_list)
            fake_btn.destroy()    
            self.geometry('%dx%d+%d+%d' % (req_width, req_height, x_pos, y_pos))
            self.overrideredirect(True)
            self.attributes("-topmost", True) # Keep the popup on top of the main window
            # Container for scrollable items
            self.frame = ctk.CTkScrollableFrame(self, width=max_wid, corner_radius=10) 
            self.frame.pack(fill="both", expand=True, padx=(10, 10), pady=(10, 10))
            menu_items=[] 
            for i in range(0, len(caption_list)):# Get Max Text Length For Menu Width
                menu_items.append([i])
                menu_items[i] = ctk.CTkButton(self.frame, text=caption_list[i], anchor="w", compound="left", fg_color="transparent", hover_color="#ff8080",  
                                            text_color=("#000000", "#ffffff"), font=self.menu_font, corner_radius=10, command=command_list[i])
                menu_items[i].pack(side='top', anchor='w', pady=0)
                menu_items[i].bind("<Button 1>", lambda e: self.exit())
...
class Calculator(ctk.CTk):
    def __init__(self, *args, **kwargs):
        ...
        self.temp_font=ctk.CTkFont(family='Arial', size=font_sizes[0], weight='bold', slant='italic')# F-C, C-F, Ratio Buttons
                ...
                if data["11"]!=str(self.screen_width) or data["12"]!=str(self.screen_height) or data["13"]!=str(self.scale_factor):
                    ...
                    if data["0"]=="":
                        while self.Language.get()=="":
                            lang = MyDialog(parent=self, dialog_type="entry", title=title, prompt=msg, choices=["English", "Spanish"], icon="setup")
        ...
        self.Display_Text.bind("<Control-c>", lambda event:self.copy_to_clipboard(app, self.Display_Text, "selected", event)) 
        ...
        self.ftoc_btn = ctk.CTkButton(self.pad_fra, text="°F→°C", anchor="c", border_width=2, corner_radius=5, font=self.temp_font,
                    border_color="black", fg_color=("#ff9c95", "#ff9c95"), hover_color="#00ced1", text_color="black",
                    command=lambda i="°F→°C": self.temp_clicked(i))  
        self.ftoc_btn.place(relx=0.01, rely=0.855, relwidth=0.117, relheight=0.12,)
        self.ftoc_btn.bind('<Button-3>', lambda event, txt="°F→°C": self.button_popups(event, txt))
        self.ctof_btn = ctk.CTkButton(self.pad_fra, text="°C→°F", anchor="c", border_width=2, corner_radius=5, font=self.temp_font,
                    border_color="black", fg_color=("#ff9c95", "#ff9c95"), hover_color="#00ced1", text_color="black",
                    command=lambda i="°C→°F": self.temp_clicked(i))  
        self.ctof_btn.place(relx=0.137, rely=0.855, relwidth=0.117, relheight=0.12)
        self.ctof_btn.bind('<Button-3>', lambda event, txt="°C→°F": self.button_popups(event, txt))
        self.ratio_btn = ctk.CTkButton(self.pad_fra, text="A : B", anchor="c", border_width=2, corner_radius=5, font=self.temp_font,
                    border_color="black", fg_color=("whitesmoke", "whitesmoke"), hover_color="#00ced1", text_color="black",
                    command=lambda i="A : B": do_ratios(i))  
    ...
    def about(self):
        ...
        info_dialog=MyDialog(parent=self, dialog_type="msgbox", title=title, prompt=msg, icon="info")
        self.grab_release()
    def precision(self, which): # Calculator Precision Configuration
        ...
        if which=='dp':
            dpp=MyDialog(self, title=titles[0], dialog_type="entry", prompt=messages[0], choices=None, init_val=mp.dps, min_val=1, max_val=500, icon="setup")
            ...
            rnd=MyDialog(self, title=titles[1], dialog_type="entry", prompt=messages[1], choices=None, init_val=self.Round.get(), min_val=0, max_val=500, icon="setup")
            if rnd.result is not None:
                while int(rnd.result) < 0 or int(rnd.result) > 500: 
                    rnd=MyDialog(self, title=titles[1], dialog_type="entry", prompt=messages[1], choices=None, init_val=self.Round.get(), min_val=0, max_val=500, icon="setup")
                self.Round.set(int(rnd.result))
        elif which=='exp':        
            snp=MyDialog(self, title=titles[2], dialog_type="entry", prompt=messages[2], choices=None, init_val=self.Exp_Precision.get(), min_val=1, max_val=500, icon="setup")
            ...
            enp=MyDialog(self, title=titles[3], dialog_type="entry", prompt=messages[3], choices=None, init_val=self.Exp_Digits.get(), min_val=1, max_val=4, icon="setup")
            if enp.result is not None:
                while int(enp.result) < 1 or int(enp.result) > 4: 
                    enp=MyDialog(self, title=titles[3], dialog_type="entry", prompt=messages[3], choices=None, init_val=self.Exp_Digits.get(), min_val=1, max_val=4, icon="setup")
                self.Exp_Digits.set(int(enp.result))
        elif which=='bit_size':
            bbs=MyDialog(self, title=titles[4], dialog_type="entry", prompt=messages[4], choices=['8','16','32','64','128','256','512'], init_val=self.Bit_Size.get(), min_val=None, max_val=None, icon="setup")
    ...
    def choose_font(self): # Calculator Display Font
        try:
            ...
            if chooser.new_font != '': 
                ...
                self.NewFont.set(Display_Font)
                self.Display_Text.focus_force()
                self.Display_Text.configure(font=Display_Font)
        except Exception as e:
            print(e)
    def choose_theme(self, theme):
        ...
        theme=MyDialog(self, title=" < Set Color Theme >", dialog_type="entry", prompt=msg, choices=["System", "Dark", "Light"], init_val=self.Theme.get(), icon="setup")
    ...
    def log_clicked(self, text):
        ...
        try:       
            ...
            cancel_dialog=MyDialog(parent=app, dialog_type="msgbox", title=title, prompt=msg, icon="cancel")
            return 'break'
    def temp_clicked(self, text):# C = K - 273.15, C = (R - 491.67) * (5 / 9), C = (F - 32) * (5/9), F = 32 + (C / (5/9)) 
        base=self.Base.get()
        if base!='Decimal':return
        if not self.Disp_List:return
        try:
    ...
    def equal_clicked(self, event):
        ...
        if self.Disp_List[-1]==' = ':return
        expr=self.Expression.get()
        self.close_brackets('all')# (Just In Case) Close All Opened Brackets Before Preceding
        sub_dict={}
        ...
        if not self.Answer_Present.get():
            try:
                constant=False
                expr=self.Expression.get()
                if expr in self._MyConstants: constant=True
                exp=parse_expr(expr, evaluate=True)
                expr_answer=exp.evalf(mp.dps, subs=sub_dict)
                if not constant:answer=self.nround_answer(expr_answer)
                else:
                ...
                if answer!='' or answer!=None:# Required 2 Entries (= And Answer)
                    ...
                    if newval!='Invalid Type!'and newval!='Type is Imaginary Literal!':
                        ...
                        self.disp_update(self.Answer.get())
    ...
    def button_popups(self, event, txt):# display the popup menu
        ...
        btn_popup = TimedPopup(self.Display_Text, text=text, text_color="#000000", fg_color="#14fafa", 
                            font_size=12, delay_time=1000, x_pos=None, y_pos=None)
    def copy_to_clipboard(self, parent, widget, which, event=None) -> str:
        ...
        try:# Ctrl+c Copy Selected Display Text To Clip-Board
            ...
            if text!="":
                text_wid_pixels = self.formula_font.measure(txt)
                display_wid=self.Display_Text.winfo_width()
                display_center_x=display_wid/2
                text_center_x=display_center_x-(text_wid_pixels/2)
                x=abs(text_center_x/display_wid)
                display_hgt=self.Display_Text.winfo_height()
                display_center_y=(display_hgt/2)
                y=abs(((display_center_y*display_hgt)/self.height)/100)
                txt=f" {txt} "#Add Spaces
                pyperclip.copy(text)
                clipboard_msg = ctk.CTkLabel(parent,  text=txt, font=self.formula_font, anchor="center",
    ...
    def show_menu_popup(self, event):# app
        ...
        commands=[lambda arg='dp': self.precision(arg), lambda arg='round': self.precision(arg), lambda arg='exp': self.precision(arg), lambda arg='exp_digits': self.precision(arg),
                        ...
                        lambda arg1=app,arg2=self.Display_Text,arg3='all': self.copy_to_clipboard(arg1 ,arg2 ,arg3), lambda arg=True: self.restart_program(arg), lambda: self.about()]
        # Get the coordinates to position the new window
        x = self.winfo_pointerx()
        y = self.winfo_pointery()
        self.popup_menu = CustomPopupMenu(self, captions ,commands, 16, x, y)
    ...
    def read_setup(self):
        ...
        try:
            ...
            if self.Language.get()=="English":
                ...
                msg1=f'Error Reading {key}, {value} In Setup.json:\n'
            else:
                title='Error al leer el archivo Setup.json\n'
                msg1=f'Error al leer {key}, {value} en Setup.json:'
            msg2= repr(e)
            msg=msg1+msg2
            cancel_dialog=MyDialog(parent=app, dialog_type="msgbox", title=title, prompt=msg, icon="cancel")
            pass
    ...
    def give_greeting(self, status):
        ...
        self.greet_popup = TimedPopup(self. Display_Text, text=greeting, text_color="#000000", fg_color="#14fafa", 
                            font_size=12, delay_time=1500, x_pos=None, y_pos=None)

if __name__ == "__main__":
    if os.path.exists("Setup.json"):# Get Color Theme
        with open('Setup.json', 'r') as json_file:
            data = json.load(json_file)
            json_file.close()
    try:        
